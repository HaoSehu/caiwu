...
class AdminUserServiceUpstreamBindingTest extends TestCase
{
    ...
    public function test_manual_service_can_bind_mofang_finance_supplier_and_override_upstream_product_id(): void
    {
        ...
        $this->assertSame(20002, $provisionData['supplier_product_id'] ?? null);
        $this->assertSame(30003, $provisionData['upstream_host_id'] ?? null);
    }

    public function test_update_upstream_host_id_only_preserves_mofang_provider_from_supplier(): void
    {
        $suffix = bin2hex(random_bytes(4));
        ...
        $this->assertSame(ProviderKey::HOSTING_PANEL_API, $provisionData['provider'] ?? null,
            'When only updating upstream_host_id, the existing provider is preserved (historical data not auto-corrected without explicit supplier rebind)');
    ...
    public function test_rebind_supplier_corrects_mismatched_provider(): void
    {
        ...
        $this->assertSame(ProviderKey::MOFANG_FINANCE_API, $provisionData['provider'] ?? null,
            'Rebinding same supplier must correct a mismatched provider to the true interface_type');
