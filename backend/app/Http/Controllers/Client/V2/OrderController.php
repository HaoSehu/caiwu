...
class OrderController extends Controller
{
    ...
    public function index(ListOrdersRequest $request)
    {
        ...
        $this->orderService->cancelExpiredPendingOrdersForUser($userId, $this->paymentWindowExpiredContext($request));
    ...
    public function show(int $id, ShowOrderRequest $request)
    {
        ...
        $this->orderService->cancelExpiredPendingOrder($order, $this->paymentWindowExpiredContext($request));
        $order = Order::query()
            ->with([
                'invoice:id,invoice_no,order_id,type,status,amount,paid_amount,paid_at,due_date,created_at',
                'service:id,name,domain,status,expires_at',
