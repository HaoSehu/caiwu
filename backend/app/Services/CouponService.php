...
class CouponService
{
    ...
    public function adminProductTree(): array
    {
        $groups = ProductCategory::query()
            ->select(['id', 'legacy_group_id', 'parent_id', 'product_type', 'name', 'sort_order'])
            ->with([
        ...
        $buildNodes = function ($group) use (&$buildNodes, $productsByGroup) {
            $publicGroupId = (int) (($group->legacy_group_id ?? 0) ?: ($group->id ?? 0));
            $children = collect($group->children ?? [])
                ->map(fn ($child) => $buildNodes($child))
                ->values()
                ->all();
    ...
    private function buildOwnedCouponPageQuery(User $user, array $filters = [], bool $applyStatusFilter = true)
    {
        ...
        $query = UserCoupon::query()
            ...
            ->selectRaw('(' . $this->ownedCouponUsageCountSql() . ') as user_used_count', [$userId, OrderStatus::CANCELLED]);
    ...
    private function applyOwnedCouponStatusFilter($query, string $statusFilter, bool $hasPlacedOrder, int $userId): void
    {
            ...
            $query->where(function ($builder) use ($hasPlacedOrder, $userId) {
                ...
                $builder->orWhere(function ($subQuery) use ($userId) {
                    $subQuery->where('coupons.per_user_limit', '>', 0)
                        ->whereRaw('(' . $this->ownedCouponUsageCountSql() . ') >= coupons.per_user_limit', [$userId, OrderStatus::CANCELLED]);
                });
            });
        ...
        $query->where(function ($builder) use ($userId) {
            $builder->whereNull('coupons.per_user_limit')
                ...
                ->orWhereRaw('(' . $this->ownedCouponUsageCountSql() . ') < coupons.per_user_limit', [$userId, OrderStatus::CANCELLED]);
        });
    }
    ...
    {
        return "SELECT COUNT(*) FROM orders WHERE orders.user_id = ? AND orders.coupon_id = user_coupons.coupon_id AND orders.status != ?";
    }
    ...
    private function buildOwnedCouponPayload(UserCoupon $userCoupon, Product $product, string $billingCycle, float $amount, ?int $userId = null, string $orderType = 'new'): array
    {
        ...
        throw_if((int) $userCoupon->status !== 1, new BusinessException('优惠券已失效'));
    ...
    private function grantCouponToUsers(Coupon $coupon, array $userIds, array $context = [], bool $replace = false): void
    {
        ...
        if ($coupon->total_usage_limit) {
            ...
            throw_if(
                $receivedCount + count($toCreate) - count($toDelete) > (int) $coupon->total_usage_limit,
    ...
    private function resolveOwnedCouponStatus(Coupon $coupon, UserCoupon $userCoupon, int $userUsedCount, bool $hasPlacedOrder): array
    {
        ...
        if ((int) $userCoupon->status !== 1 || (int) $coupon->status !== CouponStatus::ACTIVE) {
        ...
        if ($coupon->first_order_only && $hasPlacedOrder) {
            return ['status' => 'used_up', 'label' => '不可用', 'reason' => '仅限首单用户使用'];
        }
    ...
    private function normalizeAdminCouponPayload(array $payload, array $context = [], ?Coupon $coupon = null): array
    {
        ...
        $distributionType = trim((string) ($payload['distribution_type'] ?? 'public'));
        ...
        $code = $coupon?->code ?: $this->generateInternalCouponCode();
