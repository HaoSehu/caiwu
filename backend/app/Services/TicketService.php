...
class TicketService
{
    ...
    public function clientClose(Ticket $ticket, int $userId): void
    {
        throw_if($ticket->user_id !== $userId, new BusinessException('无权操作'));
        throw_if($ticket->status === self::STATUS_CLOSED, new BusinessException('工单已关闭'));
        $this->closeTicketAndReplaceAttachments($ticket);
    }
    ...
    public function detail(Ticket $ticket): array
    {
        $ticket->load([
            ...
            'service:id,name,product_id',
            'service.product:id,name',
        ...
        return [
            ...
            'service' => $ticket->service ? [
                ...
                'product_name' => $ticket->service->product?->name,
        ...
        if ($keyword !== '') {
            $query->where(function ($builder) use ($keyword) {
                $builder->where('name', 'like', '%' . $keyword . '%')
                    ...
                    ->orWhereHas('product', fn ($productQuery) => $productQuery->where('name', 'like', '%' . $keyword . '%'));
            });
        }
    ...
    private function sendTicketEmail(string $to, string $templateCode, array $params, array $context = []): void
    {
        ...
        try {
            $this->notificationService->sendTemplateEmail($email, $templateCode, $params);
        } catch (\Throwable $exception) {
            Log::warning('工单通知邮件发送失败', array_merge($context, [
                'to_email' => $email,
    ...
    public function uploadImage(int $actorId, string $actorType, UploadedFile $file): array
    {
        ...
        $attachment = $this->buildStoredAttachmentMeta(
            ...
            $file->getClientMimeType() ?: 'image/' . $extension
        );
    ...
    private function closeTicketAndReplaceAttachments(Ticket $ticket): void
    {
        $pathsToDelete = [];

        DB::transaction(function () use ($ticket, &$pathsToDelete) {
    ...
    private function resolveTicketReplies(Ticket $ticket, string $clientName): array
    {
        ...
        $attachmentsByMessage = Schema::hasTable('ticket_attachments')
        ...
        return $messages->map(function ($message) use ($attachmentsByMessage, $staffMap, $clientName) {
            ...
            return [
                ...
                'created_at' => ! empty($message->created_at)
                    ? \Illuminate\Support\Carbon::parse((string) $message->created_at)->format('Y-m-d H:i:s')
                    : null,
            ];
        })->values()->all();
    }
