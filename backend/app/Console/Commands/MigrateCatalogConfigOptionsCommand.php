...
class MigrateCatalogConfigOptionsCommand extends CatalogMigrateBaseCommand
{
    protected $signature = 'migrate:catalog:config-options
    ...
    protected $description = '从旧库 products.config_options 拆分迁移到新库 product_config_options';
    ...
    protected function migrateRows(array $commonColumns, int $batchSize): int
    {
        ...
        do {
            ...
            $payloads = [];
            foreach ($rows as $row) {
                foreach ($service->buildConfigOptions((array) $row) as $option) {
                    $payloads[] = $option;
                }
            }
            ...
            if ($payloads !== []) {
                ...
                $total += $service->batchUpsert(
                    ...
                    array_values(array_diff($columns, ['product_id', 'option_key']))
                );
            }
