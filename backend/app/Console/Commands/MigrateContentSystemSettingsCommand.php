...
class MigrateContentSystemSettingsCommand extends ContentSystemMigrateBaseCommand
{
    protected $signature = 'migrate:content-system:settings
    ...
    protected $description = '迁移旧库 settings 到 idc.settings';
    ...
    {
        return 'content_system_settings';
    }

    protected function preCheck(ContentSystemMigrationService $service): ?array
    {
        return [
            '原始配置总数' => $service->sourceCount('settings'),
            '过滤后可迁配置数' => count($service->deriveSettingPayloads()),
        ];
    }
    ...
    protected function migrateRows(array $commonColumns, int $batchSize): int
    {
        ...
        $payloads = $service->deriveSettingPayloads();
