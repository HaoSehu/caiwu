...
class MigrateServiceLifecycleLogsCommand extends ServiceMigrateBaseCommand
{
    protected $signature = 'migrate:service:lifecycle-logs
    ...
    protected $description = '迁移服务生命周期日志到 service_lifecycle_logs';
    ...
    {
        return [
            '可提取生命周期日志数' => count($service->deriveServiceLifecycleLogPayloads()),
        ];
    }
    ...
    protected function migrateRows(array $commonColumns, int $batchSize): int
    {
        ...
        $payloads = $service->deriveServiceLifecycleLogPayloads();
        ...
        foreach ($chunks as $index => $chunk) {
            $total += $service->batchUpsert(
                ...
                $columns,
                $chunk,
                ['service_instance_id', 'action', 'happened_at'],
                array_values(array_diff($columns, ['service_instance_id', 'action', 'happened_at']))
            );
