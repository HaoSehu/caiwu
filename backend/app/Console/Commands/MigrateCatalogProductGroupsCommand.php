...
class MigrateCatalogProductGroupsCommand extends CatalogMigrateBaseCommand
{
    protected $signature = 'migrate:catalog:product-groups
    ...
    protected $description = '迁移旧库 product_groups 到新库 idc.product_groups';
    ...
    protected function migrateRows(array $commonColumns, int $batchSize): int
    {
        ...
        do {
            $rows = $service->sourcePaginate('product_groups', $offset, $batchSize);
            if ($rows === []) {
                break;
            }

            $payloads = array_map(
                static fn (object $row): array => $service->buildProductGroupPayload((array) $row),
                $rows
            );
