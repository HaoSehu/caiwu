...
final class MofangFinanceAdapter implements ProvidesConsoleAccess, ProvidesConsoleCatalog, ProvidesConsoleNetwork, ProvidesConsoleRuntime, ProvidesConsoleSecurity, ProvidesProvisioning, ProvidesRenewal, ProvidesScheduledAuthRefresh, ProvidesStatusSync
{
    ...
    private readonly MofangCatalogService $catalogService;

    private readonly MofangProvisionService $provisionService;

    private readonly MofangRenewService $renewService;

    private readonly MofangStatusService $statusService;

    private readonly MofangConsoleService $consoleService;

    private readonly MofangNetworkService $networkService;

    private readonly MofangSecurityService $securityService;

    public function __construct(
        HostingPanelApiTransport $transport,
        MofangCloudConfigTemplate $cloudConfigTemplate,
        private readonly MofangProductTypeMapper $productTypeMapper = new MofangProductTypeMapper,
        ?MofangFinanceTransport $mofangTransport = null,
        ?MofangCatalogService $catalogService = null,
        ?MofangProvisionService $provisionService = null,
        ?MofangRenewService $renewService = null,
        ?MofangStatusService $statusService = null,
        ?MofangConsoleService $consoleService = null,
        ?MofangNetworkService $networkService = null,
        ?MofangSecurityService $securityService = null,
    ) {
        $this->transport = $mofangTransport ?? new MofangFinanceTransport(
            $transport,
            new MofangAuthManager($transport)
        );
        $this->catalogService = $catalogService ?? new MofangCatalogService(
            $this->transport,
            $cloudConfigTemplate,
            $this->productTypeMapper
        );
        $this->provisionService = $provisionService ?? new MofangProvisionService($this->transport);
        $this->renewService = $renewService ?? new MofangRenewService($this->transport);
        $this->statusService = $statusService ?? new MofangStatusService($this->transport);
        $this->consoleService = $consoleService ?? new MofangConsoleService($this->transport);
        $this->networkService = $networkService ?? new MofangNetworkService($this->transport, $this->consoleService);
        $this->securityService = $securityService ?? new MofangSecurityService($this->transport);
    }
