...
final class MofangAuthManager
{
    ...
    public function login(Supplier $supplier): string
    {
        ...
        if ($jwt === '') {
            $this->safeLog('warning', '[魔方财务接口] JWT响应缺少会话', [
        ...
        $this->safeLog('info', '[魔方财务接口] JWT缓存写入', [
            'supplier_id' => $supplier->id,
            'provider_key' => ProviderKey::MOFANG_FINANCE_API,
    ...
    public function refreshJwt(Supplier $supplier): string
    {
        ...
        if ($jwt === '') {
            $this->safeLog('warning', '[魔方财务接口] JWT刷新响应缺少会话', [
                'supplier_id' => $supplier->id,
                'provider_key' => ProviderKey::MOFANG_FINANCE_API,
                'response_status' => $response['status'] ?? $response['code'] ?? null,
            ]);
        ...
        $this->safeLog('info', '[魔方财务接口] JWT强制刷新', [
            'supplier_id' => $supplier->id,
            'provider_key' => ProviderKey::MOFANG_FINANCE_API,
            'cache_store' => $this->cacheStoreName(),
            'ttl_seconds' => $ttlSeconds,
    ...
    private function jwtCache(): CacheRepository
    {
        ...
        try {
            ...
            $this->safeLog('warning', '[魔方财务接口] JWT缓存仓库不可用，回退默认缓存仓库', [
                'store' => $store,
    ...
    {
        $mofangConfig = config('mofang.finance_api', []);
        $hostingConfig = config('idc.hosting_panel_api', []);

        return trim((string) (
            (is_array($mofangConfig) ? ($mofangConfig['jwt_cache_store'] ?? null) : null)
