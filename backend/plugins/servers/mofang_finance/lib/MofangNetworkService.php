...
final class MofangNetworkService
{
    ...
    private function assertUpstreamSuccess(array $response, array $allowedStatuses, string $action): void
    {
        ...
        Log::warning('[魔方财务控制台] 返回失败', [
            'action' => $action,
            'status' => $status,
            'message' => SensitiveDataSanitizer::sanitizeText($message),
        ]);

        throw new BusinessException(app(ProviderErrorMapper::class)->toUserMessage(ProviderKey::MOFANG_FINANCE_API, $action, $message), 42200);
    }
