...
function createBindingRecord(sourceValue: unknown): BindingOption | null {
  ...
  const categoryName = String(source.category_full_name || source.group_full_name || '').trim();
  return {
...
export function flattenProductTree(nodes: unknown[], result: BindingOption[] = []) {
  ...
  nodes.forEach((nodeValue) => {
    ...
    if (Array.isArray(node.children)) {
      flattenProductTree(node.children, result);
    }
  });
  return result;
}
...
function buildBindingCategoryTreeNode(
  ...
  sourceChildren.forEach((childValue, index) => {
    ...
    const record = isProductNode ? createBindingRecord(child) : null;
    ...
    const categoryNode = buildBindingCategoryTreeNode(child, `${nodeKey}-${index}`, mode);
  ...
  return {
    value: String(node.value || node.id || nodeKey),
    label: label || '未命名分类',
    text: label || '未命名分类',
    selectable,
    children: result,
...
export function buildBindingTreeOptions(nodes: unknown[], _parentKey = 'root', mode: BindingTreeMode = 'multiple'): BindingTreeNode[] {
  ...
  nodes.forEach((nodeValue, index) => {
    ...
    const typeLabel = String(
      node.product_type_label || PRODUCT_TYPE_LABELS[productType] || productType,
    ).trim();
    ...
    if (categoryNode) {
      typeNode.children = [...(typeNode.children || []), categoryNode];
    }
  });

  return Array.from(typeNodes.values()).filter((node) => (node.children || []).length > 0);
}
...
export function collectLeafProductIds(node: BindingTreeNode): string[] {
  if (!node.children || node.children.length === 0) {
    // 叶子产品节点
    return node.selectable && /^\d+$/.test(node.value) ? [node.value] : [];
  }
...
export function useProductBindingTree(mode: BindingTreeMode = 'multiple') {
  const treeLoading = ref(false);
  const treeData = ref<Record<string, unknown>[]>([]);

  const flatOptions = computed(() => flattenProductTree(treeData.value));
  const optionMap = computed(() => new Map(flatOptions.value.map((item) => [item.value, item])));
  const treeOptions = computed(() => buildBindingTreeOptions(treeData.value, 'root', mode));
  ...
  function selectionToBindings(
    ...
    if (mode === 'batch') {
      ...
      // 默认模式：仅保留纯数字产品 ID
      selectedIds = rawValues.filter((id) => /^\d+$/.test(id) && Number(id) > 0);
    }
