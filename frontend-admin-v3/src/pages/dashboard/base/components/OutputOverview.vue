  ...
  <t-card :bordered="false">
    <t-row>
      <t-col :xs="12" :xl="9">
        ...
        >
          <template #actions>
            <div class="card-date-picker-pair">
              <t-date-picker
                class="card-date-picker-container"
                v-model="stokeStartDate"
                clearable
                theme="primary"
                mode="date"
                format="YYYY-MM-DD"
                ...
                placeholder="开始日期"
                @change="onStokeDateChange"
              />
              <t-date-picker
                class="card-date-picker-container"
                v-model="stokeEndDate"
                clearable
                theme="primary"
                mode="date"
                format="YYYY-MM-DD"
...
const stokeStartDate = ref(LAST_7_DAYS[0]);
const stokeEndDate = ref(LAST_7_DAYS[1]);
...
const onStokeDateChange = () => {
  onStokeDataChange([stokeStartDate.value, stokeEndDate.value].filter(Boolean));
};
</script>
<style lang="less" scoped>
.card-date-picker-pair {
  display: flex;
  flex-wrap: wrap;
