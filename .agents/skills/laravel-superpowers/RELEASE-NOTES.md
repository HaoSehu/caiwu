...
Full Laravel 13 support based on a sweep of the official 13.x release notes and upgrade guide (released March 17, 2026; PHP 8.3–8.5).

### Added (Skills + Commands)

- **Laravel AI SDK** (`laravel:ai-sdk`) — first-party AI primitives: agents, embeddings, images, audio, tool calling, provider failover, and testing guidance
- **Semantic / Vector Search** (`laravel:vector-search`) — pgvector columns, `whereVectorSimilarTo()`, embedding lifecycle, hybrid search patterns
- **First-Party PHP Attributes** (`laravel:php-attributes`) — `#[Middleware]`, `#[Authorize]`, `#[Tries]`, `#[Backoff]`, `#[Timeout]`, `#[FailOnTimeout]`, `#[DateFormat]`, and style trade-offs
- **Request Forgery Protection** (`laravel:request-forgery-protection`) — origin-aware CSRF via `PreventRequestForgery`, `originOnly` mode, webhook exclusions
- **Upgrade to Laravel 13** (`laravel:upgrade-13`) — breaking-change checklist (CSRF middleware rename, `serializable_classes`, cache prefix changes, `upsert()` validation, event property renames) and verification steps

Matching command wrappers: `laravel-ai-sdk`, `laravel-vector-search`, `laravel-attributes`, `laravel-csrf`, `laravel-upgrade-13`.

### Changed (Laravel 13+ sections in existing skills)

- Queues & Horizon: `Queue::route()`, job attributes, `Interruptible` jobs, `--stop-when-empty-for`
- Performance Caching: `Cache::touch()`, `storage` cache driver, `serializable_classes` default, prefix migration note
- Task Scheduling: `Schedule::group()` lifecycle callbacks, `withoutOverlapping(releaseOnSignal: true)`, `schedule:list --environment`
- API Resources & Pagination: first-party JSON:API resources
- Policies & Authorization: `#[Authorize]` / `#[Middleware]` controller attributes
- Form Requests & Validation: `FailOnUnknownFields` strict mode, `Rule::jsonSchema()`, `whenFilledEnum()`
- Eloquent Relationships: collection serialization now restores eager-loaded relations; model-level attributes
- Transactions: `upsert()` empty-`uniqueBy` now throws
- Blade Components: `@fonts` directive
- Migrations & Factories: `Schema::hasForeignKey()`, `vector()` columns
- Daily Workflow: `php artisan dev` one-command dev loop

### Docs

- `docs/laravel/DERIVED_PATTERNS.md` updated to cover the 11.x–13.x intersection plus the 13.x-specific skills map
- README gains a Laravel 13 Support section; prompting-skill examples and docs links refreshed from 11.x to 13.x
