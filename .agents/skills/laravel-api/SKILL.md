...
Use this skill as a reference, not as an architecture replacement.

In an existing repository:

1. Read and obey the local `AGENTS.md`, API documentation, route files, and project-specific skills first.
2. Reuse existing routing, authentication, response builders, FormRequest classes, Resources, service layers, status maps, and validation commands.
3. Do not initialize a new API structure, remove frontend code, introduce JWT, introduce new versioning, or create parallel request/response abstractions unless the user explicitly asks for that migration.
4. Do not invent endpoints or response contracts. Query the existing API inventory, route definitions, and controllers before changing or adding API behavior.

For the Caiwu repository specifically:

- Use Laravel 12 and Sanctum token auth; do not introduce JWT.
- Successful API responses must keep `code = 0`.
- Preserve the existing pagination shape: `list`, `total`, `page`, `page_size`.
- Use `App\Traits\ApiResponse` / `App\Support\ApiResponseBuilder`, existing FormRequest classes, Resources, controllers, and services.
- Check `文档/开发文档/后端/后端API清单.md` and `文档/开发文档/后端/API清单导航.md` before API work.
- Do not hand-edit generated API inventory; regenerate it with `php backend/scripts/export_api_inventory.php` when needed.
- Keep payment records and third-party payment boundaries intact.
