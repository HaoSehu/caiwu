...
export function isCpuConfigKey(key = "", label = "") {
  ...
  return CPU_KEYWORDS.some((keyword) =>
...
export function isMemoryConfigKey(key = "", label = "") {
  const text = `${key}${label}`.toLowerCase();
  return MEMORY_KEYWORDS.some((keyword) =>
    text.includes(String(keyword).toLowerCase()),
  );
}
...
export function parseMachineSpecFromText(value) {
  ...
  const cpuMatch = compact.match(
    /(\d+(?:\.\d+)?)(?:核|核CPU|vCPU|vcpu|C(?![a-z]))/i,
  );
  const memMatch = compact.match(
    /(\d+(?:\.\d+)?)(?:GiB|gib|GB|G|MiB|mib|MB|M)(?:内存|RAM|ram)?/i,
  );
...
export function resolveMachineSpecSelection(
  configOptions,
  selectedConfig = {},
) {
  const options = Array.isArray(configOptions) ? configOptions : [];
  const cpuRaw = resolveConfigOptionSelectionLabel(
    options,
    isCpuConfigKey,
    selectedConfig.cpu,
  );
  const memoryRaw = resolveConfigOptionSelectionLabel(
    options,
    isMemoryConfigKey,
  ...
  return {
    ...
    memoryText: memoryRaw
      ? normalizeMemorySpecText(memoryRaw) ||
        parseMachineSpecFromText(memoryRaw).memoryText ||
        ""
      : "",
  };
}

export function buildMachineSpecDisplayName({
  combinedDisplayName,
  displayName,
  cpuText,
  memoryText,
...
export function resolveMachineSpecPresentation(product, selectedConfig = {}) {
  ...
  const configSpec = resolveMachineSpecSelection(
    sourceProduct.config_options,
    selectedConfig,
  );
  ...
  const specName =
    ...
    buildInstanceSpecName(
      specSource,
      parsedSpec,
      sourceProduct.id || sourceProduct.product_id || 0,
    );
...
function resolveConfigOptionSelectionLabel(
  configOptions,
  matcher,
  selectedValue,
) {
  ...
  if (normalizedSelectedValue) {
    const matchedEntry = entries.find(
      (entry) =>
        normalizeComparableValue(entry.id) ===
          normalizeComparableValue(normalizedSelectedValue) ||
        normalizeComparableValue(entry.value) ===
          normalizeComparableValue(normalizedSelectedValue) ||
        normalizeComparableValue(entry.label) ===
          normalizeComparableValue(normalizedSelectedValue),
    );
...
function buildConfigOptionEntries(option) {
  ...
  return parseParamOptions(option?.parameter)
    .map((item) => ({
      id: String(item?.id || "").trim(),
...
function hasMachineSpecSuffix(value) {
  const text = String(value || "")
    ...
    .toLowerCase();
  return (
    /(?:^|[-_])\d+(?:\.\d+)?vcpu(?:[-_]\d+(?:\.\d+)?(?:gib|mib|tib|g|m|t))?/.test(
      text,
