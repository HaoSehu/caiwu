...
export function useRecharge() {
    ...
    try {
      ...
      if (response.data?.paid) {
        ...
        rechargeSummary.value = {
          ...rechargeSummary.value,
          balance: formatMoney(response.data?.balance ?? userStore.info?.balance ?? rechargeSummary.value.balance),
        }
  ...
  function startAutoPolling(interval = 2000) {
    stopAutoPolling()
    if (!paymentPayload.value?.payment_no || !paymentPayload.value?.poll_token) {
      return
    }
