...
const handleLogin = async () => {
  ...
  try {
    ...
    if (isCaptchaRequiredError(error)) {
      try {
        ...
        if (isCaptchaRequiredError(captchaError)) {
          ElMessage.error(toUserMessage(captchaError?.message, '行为验证未通过，请重试'))
          return
        }

        if (!captchaError?.__handled) {
          ElMessage.error(toUserMessage(captchaError?.message, '登录失败'))
        }
        return
      }
    }
    ...
    if (!error?.__handled) {
      ElMessage.error(toUserMessage(error?.message, '登录失败'))
    }
  } finally {
    loading.value = false
  }
