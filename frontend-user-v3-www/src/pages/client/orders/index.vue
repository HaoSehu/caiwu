    ...
    <div v-else class="record-mobile-list" v-loading="loading">
      <div v-for="row in list" :key="row.id" class="record-mobile-card" @click="goToInvoice(row)">
        <div class="record-card-row">
          <span class="record-card-no">{{ row.order_no || `#${row.id}` }}</span>
          <el-tag :type="resolveOrderTagType(row.status)" size="small" effect="light">
            {{ row.status_label || '--' }}
          </el-tag>
        </div>
        <div class="record-card-title">{{ row.product_name || row.service_name || '--' }}</div>
        <div class="record-card-sub">{{ row.invoice_no || '未关联账单' }}</div>
        <div class="record-card-row">
          <span class="record-card-amount">订单：<strong>¥{{ row.amount || '0.00' }}</strong></span>
          <span class="record-card-time">{{ row.created_at || '--' }}</span>
        </div>
      </div>
    </div>
