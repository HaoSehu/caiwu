...
**请求方法**：GET  
**请求路径**：`/api/admin/logs/runtime/summary`  
**调试状态**：✅ 通过
...
| data.warnings | integer | 真实调用返回字段 |
| data.infos | integer | 真实调用返回字段 |
...
· 响应状态码：200  
· 调用方式：GET /api/admin/logs/runtime/summary  
· 验证方式：真实调用；接口被判定为无破坏性或仅影响本轮临时 token  
· 脱敏说明：token、password、secret、key、authorization 等敏感字段已脱敏

### 源码依据
· 控制器动作：`App\Http\Controllers\Admin\LogController@runtimeLogsSummary`  
