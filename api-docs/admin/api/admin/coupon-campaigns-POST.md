...
**请求方法**：POST  
**请求路径**：`/api/admin/coupon-campaigns`  
**调试状态**：⬜ 待调试
...
| weekdays | array | 是 | 请求体参数；校验规则：required\|array\|min:1；来源：StoreRequest |
| weekdays.* | integer | 否 | 请求体参数；校验规则：integer\|between:0,6；来源：StoreRequest |
| trigger_time | string | 是 | 请求体参数；校验规则：required\|string\|regex:/^\d{2}:\d{2}(:\d{2})?$/；来源：StoreRequest |
| issue_quantity | integer | 是 | 请求体参数；校验规则：required\|integer\|min:1；来源：StoreRequest |
| valid_duration_hours | integer | 否 | 请求体参数；校验规则：nullable\|integer\|min:1\|max:87600；来源：StoreRequest |
...
| discount_value | number | 是 | 请求体参数；校验规则：required\|numeric\|min:0.01；来源：StoreRequest |
...
| per_user_limit | integer | 否 | 请求体参数；校验规则：nullable\|integer\|min:1；来源：StoreRequest |
...
### 源码依据
· 控制器动作：`App\Http\Controllers\Admin\CouponCampaignController@store`  
