...
**请求方法**：POST  
**请求路径**：`/api/admin/suppliers/{supplier}/actions/{action}`  
**调试状态**：⬜ 待调试
...
| action | string | 是 | 路径参数；来自路由占位 `{action}` |
| payload | array | 否 | 请求体参数；校验规则：nullable\|array；来源：SupplierPluginActionRequest |
...
### 源码依据
· 控制器动作：`App\Http\Controllers\Admin\SupplierController@executePluginAction`  
