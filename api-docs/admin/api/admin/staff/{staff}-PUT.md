...
| username | string | 否 | 请求体参数；校验规则：missing；来源：UpdateAdminStaffRequest |
| nickname | string | 否 | 请求体参数；校验规则：nullable\|string\|max:50；来源：UpdateAdminStaffRequest |
| email | string | 否 | 请求体参数；校验规则：missing；来源：UpdateAdminStaffRequest |
| role_id | integer | 是 | 请求体参数；校验规则：required\|integer\|exists:roles,id；来源：UpdateAdminStaffRequest |
| status | integer | 是 | 请求体参数；校验规则：required\|integer\|in:0,1；来源：UpdateAdminStaffRequest |
...
### 源码依据
· 控制器动作：`App\Http\Controllers\Admin\AdminStaffController@update`  
