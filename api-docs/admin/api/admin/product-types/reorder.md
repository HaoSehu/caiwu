...
**请求方法**：POST  
**请求路径**：`/api/admin/product-types/reorder`  
**调试状态**：⬜ 待调试
...
| 参数名 | 类型 | 必填 | 说明 |
|---|---|---|---|
| values | array | 是 | 请求体参数；校验规则：required\|array\|min:2；来源：ReorderRequest |
| values.* | string | 是 | 请求体参数；校验规则：required\|string\|max:50\|distinct；来源：ReorderRequest |
...
### 源码依据
· 控制器动作：`App\Http\Controllers\Admin\ProductTypeController@reorder`  
