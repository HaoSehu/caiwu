...
| 参数名 | 类型 | 必填 | 说明 |
|---|---|---|---|
| name | string | 否 | 请求体参数；校验规则：nullable\|string\|max:255；来源：StoreRequest |
| display_name | string | 否 | 请求体参数；校验规则：nullable\|string\|max:255；来源：StoreRequest |
| custom_display_name | string | 否 | 请求体参数；校验规则：nullable\|string\|max:255；来源：StoreRequest |
| product_type | string | 否 | 请求体参数；校验规则：nullable\|string\|max:50；来源：StoreRequest |
| service_type_code | string | 否 | 请求体参数；校验规则：nullable\|string\|max:50；来源：StoreRequest |
| first_product_group_id | integer | 否 | 请求体参数；校验规则：nullable\|integer\|min:1；来源：StoreRequest |
| second_product_group_id | integer | 是 | 请求体参数；校验规则：required\|integer\|min:1；来源：StoreRequest |
| third_product_group_id | integer | 否 | 请求体参数；校验规则：nullable\|integer\|min:1；来源：StoreRequest |
| remark | string | 否 | 请求体参数；校验规则：nullable\|string\|max:255；来源：StoreRequest |
| pricing | array | 是 | 请求体参数；校验规则：required\|array；来源：StoreRequest |
| pricing.monthly | number | 否 | 请求体参数；校验规则：nullable\|numeric\|min:0；来源：StoreRequest |
| pricing.quarterly | number | 否 | 请求体参数；校验规则：nullable\|numeric\|min:0；来源：StoreRequest |
| pricing.semiannually | number | 否 | 请求体参数；校验规则：nullable\|numeric\|min:0；来源：StoreRequest |
| pricing.annually | number | 否 | 请求体参数；校验规则：nullable\|numeric\|min:0；来源：StoreRequest |
| setup_fee | number | 否 | 请求体参数；校验规则：nullable\|numeric\|min:0；来源：StoreRequest |
| config_options | array | 否 | 请求体参数；校验规则：nullable\|array；来源：StoreRequest |
| purchase_requires | array | 否 | 请求体参数；校验规则：nullable\|array；来源：StoreRequest |
| purchase_requires.require_verification | boolean | 否 | 请求体参数；校验规则：nullable\|boolean；来源：StoreRequest |
| purchase_requires.require_phone | boolean | 否 | 请求体参数；校验规则：nullable\|boolean；来源：StoreRequest |
| purchase_requires.provision_hostname | array | 否 | 请求体参数；校验规则：nullable\|array；来源：StoreRequest |
| purchase_requires.provision_hostname.mode | string | 否 | 请求体参数；校验规则：nullable\|string\|in:"system","fixed","prefix"；来源：StoreRequest |
| purchase_requires.provision_hostname.value | string | 否 | 请求体参数；校验规则：nullable\|string\|max:200；来源：StoreRequest |
| purchase_requires.provision_hostname.length | integer | 否 | 请求体参数；校验规则：nullable\|integer\|min:4\|max:63；来源：StoreRequest |
| stock | integer | 否 | 请求体参数；校验规则：nullable\|integer\|min:-1；来源：StoreRequest |
...
| auto_setup | string | 否 | 请求体参数；校验规则：nullable\|in:0,1；来源：StoreRequest |
| upstream_binding | array | 否 | 请求体参数；校验规则：nullable\|array；来源：StoreRequest |
| upstream_binding.supplier_id | integer | 否 | 请求体参数；校验规则：nullable\|integer\|exists:suppliers,id；来源：StoreRequest |
| upstream_binding.upstream_product_id | string | 否 | 请求体参数；校验规则：nullable；来源：StoreRequest |
...
### 源码依据
· 控制器动作：`App\Http\Controllers\Admin\ProductController@store`  
