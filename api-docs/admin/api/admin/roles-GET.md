...
| 参数名 | 类型 | 必填 | 说明 |
|---|---|---|---|
| keyword | string | 否 | 查询参数；校验规则：nullable\|string\|max:100；来源：ListAdminRolesRequest |
...
| data.list.stored_permissions | array | 真实调用返回字段 |
| data.list.admin_count | integer | 真实调用返回字段 |
| data.list.is_builtin | boolean | 真实调用返回字段 |
| data.list.is_locked | boolean | 真实调用返回字段 |
| data.list.can_edit_permissions | boolean | 真实调用返回字段 |
    ...
    "data": {
        "list": [
            ...
            },
            {
                ...
                "can_edit_permissions": false,
                "can_delete": false,
                "created_at": "2026-07-03 12:48:13",
                "updated_at": "2026-07-05 16:35:15"
            },
            {
                "id": 4,
...
· 响应状态码：200  
· 调用方式：GET /api/admin/roles  
· 验证方式：真实调用；接口被判定为无破坏性或仅影响本轮临时 token  
· 脱敏说明：token、password、secret、key、authorization 等敏感字段已脱敏

### 源码依据
· 控制器动作：`App\Http\Controllers\Admin\AdminRoleController@index`  
