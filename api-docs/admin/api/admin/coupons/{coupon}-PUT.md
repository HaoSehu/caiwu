...
| 参数名 | 类型 | 必填 | 说明 |
|---|---|---|---|
| coupon | string | 是 | 路径参数；来自路由占位 `{coupon}` |
| name | string | 是 | 请求体参数；校验规则：required\|string\|max:120；来源：UpdateRequest |
| code | string | 否 | 请求体参数；校验规则：nullable\|string\|max:50\|unique:coupons,code,NULL,id；来源：UpdateRequest |
...
| discount_value | number | 是 | 请求体参数；校验规则：required\|numeric\|min:0；来源：UpdateRequest |
| distribution_type | string | 是 | 请求体参数；校验规则：required\|in:"public","private"；来源：UpdateRequest |
...
| user_ids | array | 否 | 请求体参数；校验规则：nullable\|array；来源：UpdateRequest |
| user_ids.* | integer | 否 | 请求体参数；校验规则：integer\|exists:users,id；来源：UpdateRequest |
| first_order_only | boolean | 否 | 请求体参数；校验规则：nullable\|boolean；来源：UpdateRequest |
| total_usage_limit | integer | 否 | 请求体参数；校验规则：nullable\|integer\|min:0；来源：UpdateRequest |
| per_user_limit | integer | 否 | 请求体参数；校验规则：nullable\|integer\|min:0；来源：UpdateRequest |
...
| starts_at | string(datetime) | 否 | 请求体参数；校验规则：nullable\|date；来源：UpdateRequest |
| expires_at | string(datetime) | 否 | 请求体参数；校验规则：nullable\|date\|after_or_equal:starts_at；来源：UpdateRequest |
...
### 源码依据
· 控制器动作：`App\Http\Controllers\Admin\CouponController@update`  
