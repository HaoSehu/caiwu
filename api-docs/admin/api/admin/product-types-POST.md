...
| 参数名 | 类型 | 必填 | 说明 |
|---|---|---|---|
| label | string | 是 | 请求体参数；校验规则：required\|string\|max:30；来源：StoreRequest |
| icon | string | 否 | 请求体参数；校验规则：nullable\|string\|max:50；来源：StoreRequest |
...
### 源码依据
· 控制器动作：`App\Http\Controllers\Admin\ProductTypeController@store`  
