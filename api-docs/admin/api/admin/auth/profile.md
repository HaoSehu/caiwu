...
| 参数名 | 类型 | 必填 | 说明 |
|---|---|---|---|
| nickname | string | 否 | 请求体参数；校验规则：nullable\|string\|max:50；来源：UpdateProfileRequest |
| email | string | 否 | 请求体参数；校验规则：missing；来源：UpdateProfileRequest |
...
### 源码依据
· 控制器动作：`App\Http\Controllers\Admin\AuthController@updateProfile`  
