...
| 参数名 | 类型 | 必填 | 说明 |
|---|---|---|---|
| product_ids | array | 是 | 请求体参数；校验规则：required\|array\|min:1\|max:100；来源：SplitRequest |
| product_ids.* | integer | 否 | 请求体参数；校验规则：integer\|min:1\|distinct；来源：SplitRequest |
...
### 源码依据
· 控制器动作：`App\Http\Controllers\Admin\ProductController@split`  
