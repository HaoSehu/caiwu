...
| nickname | string | 否 | 请求体参数；校验规则：nullable\|string\|max:50；来源：UpdateUserRequest |
| phone | string | 否 | 请求体参数；校验规则：nullable\|string\|max:20\|closure:自定义校验\|unique:users,phone,NULL,id；来源：UpdateUserRequest |
| password | string | 否 | 请求体参数；校验规则：nullable\|string\|min:6；来源：UpdateUserRequest |
| status | string | 否 | 请求体参数；校验规则：nullable\|in:0,1；来源：UpdateUserRequest |
| credit_limit | number | 否 | 请求体参数；校验规则：nullable\|numeric\|min:0；来源：UpdateUserRequest |
...
### 源码依据
· 控制器动作：`App\Http\Controllers\Admin\UserController@update`  
