...
**请求方法**：POST  
**请求路径**：`/api/admin/users/{user}/services/{serviceId}/refund`  
**调试状态**：⬜ 待调试
...
| refund_method | string | 是 | 请求体参数；校验规则：required\|string\|in:balance,original；来源：RefundUserServiceRequest |
| amount | number | 否 | 请求体参数；校验规则：nullable\|numeric\|min:0.01\|max:99999999；来源：RefundUserServiceRequest |
| remark | string | 是 | 请求体参数；校验规则：required\|string\|min:2\|max:200；来源：RefundUserServiceRequest |
...
| data.order_status | array | 业务字段；由源码静态提取 |
| data.refund | string | 业务字段；由源码静态提取 |
| data.already_refunded | string | 业务字段；由源码静态提取 |
...
### 源码依据
· 控制器动作：`App\Http\Controllers\Admin\UserController@refundService`  
