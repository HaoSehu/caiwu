...
**请求方法**：POST  
**请求路径**：`/api/admin/users/{user}/services/{serviceId}/power`  
**调试状态**：⬜ 待调试
...
| action | string | 是 | 请求体参数；校验规则：required\|string\|in:on,off,reboot,hard_off,hard_reboot；来源：ServicePowerRequest |
...
### 源码依据
· 控制器动作：`App\Http\Controllers\Admin\UserController@servicePower`  
