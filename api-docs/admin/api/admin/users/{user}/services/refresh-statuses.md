...
**请求方法**：POST  
**请求路径**：`/api/admin/users/{user}/services/refresh-statuses`  
**调试状态**：⬜ 待调试
...
| service_ids | array | 是 | 请求体参数；校验规则：required\|array\|min:1\|max:50；来源：RefreshServiceStatusesRequest |
| service_ids.* | integer | 否 | 请求体参数；校验规则：integer\|min:1；来源：RefreshServiceStatusesRequest |
...
### 源码依据
· 控制器动作：`App\Http\Controllers\Admin\UserController@refreshServiceStatuses`  
