...
**请求方法**：GET  
**请求路径**：`/api/admin/users/{user}/services/{serviceId}`  
**调试状态**：⚠️ 异常
...
| data.status_label | string | 服务状态文案 |
| data.billing_cycle | string | 计费周期 |
| data.amount | string | 服务金额 |
| data.expires_at | string|null | 到期时间 |
| data.provision_data | object | 开通/上游扩展数据 |
| data.connection | object|null | 连接信息；敏感字段按权限脱敏 |
...
```json
{
    ...
    "data": {
        ...
        "amount": "100.00",
        "expires_at": "2026-08-05 16:00:00",
        "provision_data": {},
        "connection": {}
    },
...
· 响应状态码：404  
· 调用方式：GET /api/admin/users/{user}/services/{serviceId}  
...
### 源码依据
· 控制器动作：`App\Http\Controllers\Admin\UserController@serviceDetail`  
