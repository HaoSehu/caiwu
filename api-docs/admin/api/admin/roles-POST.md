...
| 参数名 | 类型 | 必填 | 说明 |
|---|---|---|---|
| name | string | 是 | 请求体参数；校验规则：required\|string\|max:50\|regex:/^[A-Za-z0-9_.-]+$/\|unique:roles,name；来源：StoreAdminRoleRequest |
| label | string | 是 | 请求体参数；校验规则：required\|string\|max:50；来源：StoreAdminRoleRequest |
| permissions | array | 否 | 请求体参数；校验规则：nullable\|array；来源：StoreAdminRoleRequest |
| permissions.* | string | 否 | 请求体参数；校验规则：string\|max:100；来源：StoreAdminRoleRequest |
...
### 源码依据
· 控制器动作：`App\Http\Controllers\Admin\AdminRoleController@store`  
