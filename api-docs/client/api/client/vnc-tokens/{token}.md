...
**请求方法**：GET  
**请求路径**：`/api/client/vnc-tokens/{token}`  
**调试状态**：⬜ 待调试
...
| 参数名 | 类型 | 必填 | 说明 |
|---|---|---|---|
| token | string | 是 | 路径参数；来自路由占位 `{token}` |
...
### 源码依据
· 控制器动作：`App\Http\Controllers\Client\ServiceController@vncToken`  
· 请求校验：`根据控制器签名、FormRequest 和路由参数推断`  
· 响应结构：`统一响应外层来自 App\Support\ApiResponseBuilder；具体 data 字段以控制器、Resource、Service 返回为准`  
· 中间件：`api, throttle:30,1,client-vnc-token`
