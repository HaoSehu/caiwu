...
**请求方法**：GET  
**请求路径**：`/api/client/help-articles/{articleId}`  
**调试状态**：✅ 通过
...
| 参数名 | 类型 | 必填 | 说明 |
|---|---|---|---|
| articleId | integer\|string | 是 | 路径参数；来自路由占位 `{articleId}` |
...
```json
{
    ...
    "data": {
        ...
        "updated_at": "2026-07-05 16:35:25",
        "creator": null,
        "updater": null
    },
...
· 响应状态码：200  
· 调用方式：GET /api/client/help-articles/{articleId}  
· 验证方式：真实调用；接口被判定为无破坏性或仅影响本轮临时 token  
· 脱敏说明：token、password、secret、key、authorization 等敏感字段已脱敏

### 源码依据
· 控制器动作：`App\Http\Controllers\Client\ContentController@helpDetail`  
· 请求校验：`无 FormRequest`  
