...
| 参数名 | 类型 | 必填 | 说明 |
|---|---|---|---|
| certify_id | integer | 否 | 请求体参数；控制器通过 `$request->input()` 读取；未发现 FormRequest 明确规则 |
| order_no | string | 否 | 请求体参数；控制器通过 `$request->input()` 读取；未发现 FormRequest 明确规则 |
...
| data.certify_id | integer | 业务字段；由源码静态提取 |
