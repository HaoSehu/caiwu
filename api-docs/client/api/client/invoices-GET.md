...
| type | string | 否 | 查询参数；校验规则：nullable\|string\|closure:自定义校验；来源：IndexRequest |
...
| per_page | string | 否 | 查询参数；校验规则：prohibited；来源：IndexRequest |
...
· 响应状态码：200  
· 调用方式：GET /api/client/invoices  
· 验证方式：真实调用；接口被判定为无破坏性或仅影响本轮临时 token  
· 脱敏说明：token、password、secret、key、authorization 等敏感字段已脱敏

### 源码依据
· 控制器动作：`App\Http\Controllers\Client\InvoiceController@index`  
· 请求校验：`App\Http\Requests\Client\Invoice\IndexRequest::rules()`  
