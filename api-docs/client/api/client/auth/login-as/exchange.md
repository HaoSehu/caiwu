...
**请求方法**：POST  
**请求路径**：`/api/client/auth/login-as/exchange`  
**调试状态**：⬜ 待调试
...
| 参数名 | 类型 | 必填 | 说明 |
|---|---|---|---|
| code | string | 是 | 请求体参数；校验规则：required\|string\|min:32\|max:128；来源：ExchangeLoginAsCodeRequest |
...
### 源码依据
· 控制器动作：`App\Http\Controllers\Client\AuthController@exchangeLoginAsCode`  
· 请求校验：`根据控制器签名、FormRequest 和路由参数推断`  
· 响应结构：`统一响应外层来自 App\Support\ApiResponseBuilder；具体 data 字段以控制器、Resource、Service 返回为准`  
· 中间件：`api, throttle:10,1,client-auth-login-as`
