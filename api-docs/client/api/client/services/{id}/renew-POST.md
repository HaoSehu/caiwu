...
| billing_cycle | string | 是 | 请求体参数；校验规则：required\|string\|max:30；来源：CreateRenewOrderRequest |
| user_coupon_id | integer | 否 | 请求体参数；校验规则：nullable\|integer\|min:1；来源：CreateRenewOrderRequest |
...
### 源码依据
· 控制器动作：`App\Http\Controllers\Client\ServiceController@createRenewOrder`  
· 请求校验：`根据控制器签名、FormRequest 和路由参数推断`  
· 响应结构：`统一响应外层来自 App\Support\ApiResponseBuilder；具体 data 字段以控制器、Resource、Service 返回为准`  
· 中间件：`api, auth:sanctum, ensure.client, throttle:6,1,client-service-renew`
