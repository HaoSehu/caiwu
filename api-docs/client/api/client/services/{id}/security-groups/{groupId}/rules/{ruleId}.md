...
**请求方法**：DELETE  
**请求路径**：`/api/client/services/{id}/security-groups/{groupId}/rules/{ruleId}`  
**调试状态**：⬜ 待调试
...
| ruleId | integer\|string | 是 | 路径参数；来自路由占位 `{ruleId}` |
...
### 源码依据
· 控制器动作：`App\Http\Controllers\Client\ServiceController@deleteSecurityRule`  
· 请求校验：`根据控制器签名、FormRequest 和路由参数推断`  
· 响应结构：`统一响应外层来自 App\Support\ApiResponseBuilder；具体 data 字段以控制器、Resource、Service 返回为准`  
· 中间件：`api, auth:sanctum, ensure.client, throttle:10,1,client-service-security-rule-delete`
