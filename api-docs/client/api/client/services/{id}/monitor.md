...
**请求方法**：GET  
**请求路径**：`/api/client/services/{id}/monitor`  
**调试状态**：⬜ 待调试
...
| type | string | 否 | 查询参数；校验规则：nullable\|string\|max:100；来源：MonitorRequest |
| range | string | 否 | 查询参数；校验规则：nullable\|string\|in:3h,24h,7d,30d；来源：MonitorRequest |
| start | integer | 否 | 查询参数；校验规则：nullable\|integer\|min:0；来源：MonitorRequest |
| end | integer | 否 | 查询参数；校验规则：nullable\|integer\|min:0；来源：MonitorRequest |
...
| data.selected_type | string | 业务字段；由源码静态提取 |
| data.selected_label | string | 业务字段；由源码静态提取 |
...
### 源码依据
· 控制器动作：`App\Http\Controllers\Client\ServiceController@monitor`  
