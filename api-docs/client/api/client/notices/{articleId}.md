...
**请求方法**：GET  
**请求路径**：`/api/client/notices/{articleId}`  
**调试状态**：⚠️ 异常
...
· 响应状态码：422  
· 调用方式：GET /api/client/notices/{articleId}  
...
### 源码依据
· 控制器动作：`App\Http\Controllers\Client\ContentController@noticeDetail`  
· 请求校验：`无 FormRequest`  
· 响应结构：`统一响应外层来自 App\Support\ApiResponseBuilder；ContentArticleResource::toArray() 字段；控制器/服务/资源可静态确认 data 字段`  
