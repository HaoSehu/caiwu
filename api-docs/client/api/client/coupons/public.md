...
**请求方法**：GET  
**请求路径**：`/api/client/coupons/public`  
**调试状态**：✅ 通过
...
· 响应状态码：200  
· 调用方式：GET /api/client/coupons/public  
· 验证方式：真实调用；接口被判定为无破坏性或仅影响本轮临时 token  
· 脱敏说明：token、password、secret、key、authorization 等敏感字段已脱敏

### 源码依据
· 控制器动作：`App\Http\Controllers\Client\CouponController@publicIndex`  
· 请求校验：`App\Http\Requests\Client\Coupon\ListCouponsRequest::rules()`  
· 响应结构：`统一响应外层来自 App\Support\ApiResponseBuilder；CouponService::paginatePublicForUser() 服务返回数组字段`  
