...
| 参数名 | 类型 | 必填 | 说明 |
|---|---|---|---|
| keyword | string | 否 | 查询参数；校验规则：nullable\|string\|max:100；来源：PublishedListRequest |
| category_id | integer | 否 | 查询参数；校验规则：nullable\|integer\|exists:content_categories,id；来源：PublishedListRequest |
| content_category_id | integer | 否 | 查询参数；校验规则：nullable\|integer\|exists:content_categories,id；来源：PublishedListRequest |
| is_recommended | integer | 否 | 查询参数；校验规则：nullable\|integer\|in:"0","1"；来源：PublishedListRequest |
| page | integer | 否 | 查询参数；校验规则：nullable\|integer\|min:1；来源：PublishedListRequest |
| page_size | integer | 否 | 查询参数；校验规则：nullable\|integer\|min:1\|max:50；来源：PublishedListRequest |
...
· 响应状态码：200  
· 调用方式：GET /api/client/help-articles  
· 验证方式：真实调用；接口被判定为无破坏性或仅影响本轮临时 token  
· 脱敏说明：token、password、secret、key、authorization 等敏感字段已脱敏

### 源码依据
· 控制器动作：`App\Http\Controllers\Client\ContentController@helpArticles`  
· 请求校验：`App\Http\Requests\Client\Content\PublishedListRequest::rules()`  
