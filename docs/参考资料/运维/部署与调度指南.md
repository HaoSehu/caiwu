...
> **迁移后进程缓存提示**：`SchemaMetadataCache` 是进程内静态缓存，`gateway_logs.detail_key` 等列结构迁移后，长驻进程（octane / 常驻队列 worker）需重启或调用 `SchemaMetadataCache::flush()` 才会切换到新列行为；未切换前按旧结构走库内完整截断摘要，属安全降级而非错误。每分钟短进程的 `schedule:run` 天然无此问题。
...
- `SANCTUM_TOKEN_EXPIRATION`（分钟，默认 1440=24 小时；令牌创建超过该时长即失效，需重新登录）
- `SANCTUM_IDLE_TIMEOUT`（秒，默认 10800=3 小时；令牌超过该时长无带凭证的认证请求即失效，`last_used_at` 仅由认证接口刷新，官网公开页面浏览不会续期）
...
# 日志目录加 setgid：任何用户（含 root cron）新创建的当日日志都继承 www 组，保持组可写
...
`bootstrap/app.php` 已设置 `umask(0002)`：应用进程（含 PHP-FPM、artisan、队列与调度）新建文件为 664、目录为 775。因此当日日志（`laravel-YYYY-MM-DD.log`、`api-json-*`、`gateway-json-*`）创建即为组可写，且 Monolog 每次打开日志文件都会按配置 `chmod 0664` 修正存量文件，无需再每天手动调整权限。若日志仍出现 644，优先排查目录是否丢失 setgid、以及调度任务是否以非 `www` 组用户运行。
