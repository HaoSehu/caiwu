...
# 从 backend/.env 读取 APP_URL、FRONTEND_URL、CLIENT_CONSOLE_URL、ADMIN_URL，
...
> **本地打生产包**：本地 `backend/.env` 是开发地址（`127.0.0.1`），不要改动它来打包。生产地址放在独立的 `backend/.env.production`（已 gitignore，模板见 `backend/.env.production.example`）：

```bash
# 首次使用：复制模板并填写真实生产域名
cp backend/.env.production.example backend/.env.production

# 从 backend/.env.production 读取四个公开地址，构建三个生产 dist
npm run build:frontends:prod

# 只预览本次生产构建会用到的地址，不实际构建
node backend/scripts/build_frontends.mjs --env production --dry-run
```

生产模式下四个公开地址必须全部来自 `backend/.env.production`（或显式环境变量），缺项直接报错，绝不回退到本地 `backend/.env`，避免产出指向 `127.0.0.1` 的错误 dist。
