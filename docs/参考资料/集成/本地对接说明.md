...
| Shared Transport    | 仅作为底层 HTTP、DNS/TLS 安全、超时、日志记录（完整信息不脱敏）等共享传输辅助 | `HostingPanelApiTransport.php`                                                                                                   |
...
- 上游请求失败由插件传输入口和共享底层传输记录日志，日志必须能区分 `zjmf_finance_api` 与 `hosting_panel_api`；共享底层传输日志不做脱敏，显示完整信息
