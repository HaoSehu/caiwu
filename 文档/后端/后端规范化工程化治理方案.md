...
API 治理必须同时覆盖请求入口和响应出口。新增或重构接口时，以 `文档/后端/API格式规范.md` 为当前 API 规范文档，并以当前代码检查结果反向补充该文档。
...
- API 请求/响应规则变化时，优先更新 `文档/后端/API格式规范.md`。
- 路由清单变化时按脚本重刷 `文档/后端/后端API清单.md`，禁止手工编辑该文件。
...
- 后端文档：`文档/后端/后端规范化工程化治理方案.md`。
- API 格式：`文档/后端/API格式规范.md`。
- API 分组导航：`文档/后端/API清单导航.md`。
