...
- 兼容旧参数时，应在 FormRequest 或专用 Request DTO 中转换为内部标准字段，并在注释或文档中说明兼容原因。
...
- 用户端账单详情必须使用独立 Resource/DTO 白名单，不得复用管理端详情，不得返回 `raw_status`、`payments.*.trade_no` 等内部审计字段。
...
- 不得随意删除或重命名前端已消费字段。
- 必须变更字段时，应在报告或需求中说明影响面，并同步前端调用、接口文档和测试。
- 历史兼容字段可以保留，但应标明保留原因和移除条件。
