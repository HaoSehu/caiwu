...
- 对齐时间：`2026-07-02`
- 配套参考：
  - `文档/开发文档/集成/ZJMF 财务API文档.md` - 历史官方完整接口参考
...
| Legacy Zjmf Support | 旧密码兼容、账单恢复等非数据面兼容能力 | `backend/app/Integrations/Zjmf/Support/`、`backend/app/Integrations/Zjmf/Billing/` |
...
| 旧ZJMF密码兼容 | `backend/app/Integrations/Zjmf/Support/ZjmfLegacyPasswordVerifier.php` |
...
| ZJMF账单恢复工具 | `backend/app/Integrations/Zjmf/Billing/ZjmfBillingRestoreService.php` |
...
| 能力 | 方法 | 路径 | 本地调用入口 |
| --- | --- | --- | --- |
| ZJMF登录并换 JWT | `POST` | `/v1/login_api` | `ZjmfAuthManager::login()` / `ZjmfFinanceTransport::login()` |
| ZJMF获取用户资料 | `GET` | `/v1/user` | `ZjmfFinanceTransport::getUserProfile()` |
| ZJMF获取账户余额 | `GET` | `/v1/user`（复用） | `ZjmfFinanceTransport::getBalance()` |
| ZJMF商品、价格、库存、配置项 | 多个官方接口 | 官方商品和配置项接口 | `ZjmfCatalogService` |
| ZJMF开通 | 多个官方接口 | 购物车、结算、上游支付、host 回查 | `ZjmfProvisionService::provisionOrder()` |
| ZJMF续费 | `GET/POST/PUT` | `/v1/hosts/{hostId}/renew` 等 | `ZjmfRenewService` |
| ZJMF状态同步 | `GET` | `/v1/hosts/{hostId}` 等 | `ZjmfStatusService` |
...
| 官方文档 | 对应本地文件 |
| --- | --- |
| `文档/开发文档/集成/ZJMF 财务API文档.md` | `backend/plugins/servers/zjmf_finance/lib/ZjmfFinanceAdapter.php` + `Zjmf*Service.php` + `ZjmfFinanceTransport.php` |
