...
**新增 API：** `GET /api/admin/charge-consumption-monthly`
...
**新增 API：** `GET /api/admin/renewal-monthly`
...
**新增 API：** `GET /api/admin/expiring-services`
...
| # | 方法 | 路径 | 说明 | 权限 | 新建/增强 |
|---|------|------|------|------|:--:|
| 1 | `GET` | `/api/admin/product-income-summary` | 商品收入逐月 + 汇总 | `finance.report` | 增强 |
| 2 | `GET` | `/api/admin/charge-consumption-monthly` | 充消逐月 + 汇总 | `finance.report` | 新建 |
| 3 | `GET` | `/api/admin/renewal-monthly` | 续费逐月 + 汇总 | `finance.report` | 新建 |
| 4 | `GET` | `/api/admin/expiring-services` | 即将到期服务列表 + 统计 | `finance.report` | 新建 |
...
图表库使用 ECharts（项目可能已有，否则安装 `echarts` + `vue-echarts`）。
...
路由注册在 `backend/routes/admin.php`，挂到现有 `permission:finance.report` 组。
...
- 不引入第二套 UI 库或图表库（ECharts 如未安装需确认是否需要新增依赖）。
