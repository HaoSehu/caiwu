...
- 当前项目已经有 `PaymentGatewayManager`、`ProviderResolver`、`PluginRuntimeRegistry` 和各能力域 adapter，应在此基础上扩展。
...
| `name` | 展示名称 |
| `version` | 插件版本 |
| `provider_class` | 预留字段，当前不允许插件自行注册 ServiceProvider |
| `entry_class` | 插件入口类 |
...
当前真实上游插件为魔方财务，demo 上游插件仅用于本地开发和测试。
