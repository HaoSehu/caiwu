...
server {
    ...
    location ~* \.(js|css|html|svg|json|map|mjs)$ {
        gzip_static on;
        brotli_static on;
        expires 7d;
...
确保 Node.js 版本满足要求后，依次构建：

```bash
# 管理端
cd /www/wwwroot/你的项目/frontend-admin-v3
npm install
npm run build
# 产物目录: dist/

# 官网/用户入口
cd /www/wwwroot/你的项目/frontend-user-v3-www
npm install
npm run build
# 产物目录: dist/

# 用户控制台
cd /www/wwwroot/你的项目/frontend-user-v4-console
npm install
npm run build
# 产物目录: dist/
```
