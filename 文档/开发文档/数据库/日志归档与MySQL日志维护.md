...
```text
${LOG_ARCHIVE_ROOT}/{表名}/{表名}_YYYYMMDD.log
```

文件内容是带表头的 CSV。同一张表同一天重跑时，pt-archiver 追加到原文件且不重复写表头。执行报告和机器可读运行日志单独存放在 `LOG_ARCHIVE_REPORT_ROOT`，不与归档数据混放。
...
```dotenv
LOG_ARCHIVE_ROOT=/www/wwwroot/caiwu/backend/storage/app/private/log-archives
LOG_ARCHIVE_REPORT_ROOT=/www/wwwroot/caiwu/backend/storage/logs/log-archive
LOG_ARCHIVE_MOUNT_POINT=
LOG_ARCHIVE_CONCURRENCY=2
LOG_ARCHIVE_BATCH_SIZE=1000
LOG_ARCHIVE_SLEEP_SECONDS=1
PT_ARCHIVER_BINARY=/usr/bin/pt-archiver
PT_ARCHIVER_DEFAULTS_FILE=/etc/caiwu/pt-archiver.cnf
```

使用 NAS 时，把 `LOG_ARCHIVE_ROOT` 改为 NAS 绝对路径，并设置 `LOG_ARCHIVE_MOUNT_POINT`。命令会检查挂载状态，未挂载时直接失败，避免误写本地挂载目录。
...
- `LOG_ARCHIVE_REPORT_ROOT/run_*.json`：单次完整执行报告；
- `LOG_ARCHIVE_REPORT_ROOT/archive-YYYY-MM-DD.log`：逐事件 JSON 日志。
