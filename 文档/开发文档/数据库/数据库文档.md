...
- 文档性质：当前数据库业务说明 / 结构索引
- 对齐时间：`2026-07-08`（最后一条 migration：`2026_07_08_030000_create_message_logs_and_drop_legacy_logs`，batch 73）
...
- 图中只表达核心业务关系。真实库当前有 81 个数据库级外键，更多关系由 Eloquent 关系、业务代码和索引约定维护。
...
- 所有表已有主键。
- 所有业务表均为 `InnoDB`。
- 59 张表中部分核心表已有表注释，完整注释状态以自动结构快照为准。
...
- `php artisan db:audit-foreign-keys --strict` 用于复核所有 `*_id` 字段；当前结果为 81 个已有 FK、0 个可补 FK、50 个多态/快照/外部标识字段、0 个未分类字段。
