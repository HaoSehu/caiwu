...
- 文档性质：当前数据库业务说明 / 结构索引
- 对齐时间：`2026-07-08`（最后一条 migration：`2026_07_08_020000_add_stage_two_foreign_keys`，batch 72）
...
- `orders`、`invoices`、`payments`、`payment_callbacks`、`services`、插件配置、上游绑定、权限、内容、优惠券、返佣、工单、通知和实名历史等可补 `*_id` 均已声明数据库级外键。
- `php artisan db:audit-foreign-keys --strict` 用于复核所有 `*_id` 字段；当前结果为 83 个已有 FK、0 个可补 FK、53 个多态/快照/外部标识字段、0 个未分类字段。
