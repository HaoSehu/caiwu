...
- 支付网关第一阶段已落地 `PaymentGatewayInterface`、支付 DTO、`PaymentGatewayRegistry`、`PaymentGatewayManager` 和支付宝当面付 driver。
- `payments.gateway` 仍保留历史业务值 `alipay`，网关插件 key 使用 `alipay_f2f`，两者已通过 `PaymentGatewayCode` 常量隔离，不得混用。
- 后续新增支付方式必须新增 driver 并通过 `payment.gateways` tag 注册，不得在 `PaymentService` 或 Controller 中直接注入具体 provider。
- 实名认证已接入 `VerificationDriverManager`，driver 通过 `identity.verification_drivers` tag 注册，初始化、扫码链接、查询结果已使用 DTO，业务层不得依赖供应商原始数组。
- 短信已接入 `SmsDriverManager`，driver 通过 `sms.drivers` tag 注册，发送请求/结果已使用 DTO，供应商英文或技术类错误必须映射为简体中文业务提示。
- 上游 provider 已通过 `UpstreamDriver::capabilities()` 声明能力，`ResolvedProvider` 可输出能力列表；`mofang_finance_api` 必须继续解析到魔方 adapter，不得折叠为共享主机面板 transport。
- 上游 provider 元数据已通过 `UpstreamProviderDescriptor` 输出 key、中文 label 和能力列表；新增审查工具、配置界面或 API 扩展应优先消费 descriptor，旧 `options()` 仅作为选择器兼容结构。
- 邮件、对象存储已补能力域 contract 和 fake provider 骨架；接入真实 provider 前必须补 DTO、registry、配置注册、错误映射和 fake provider 测试。
- `IntegrationException`、`ProviderErrorMapper`、`CallbackDispatcher` 是第三方统一异常、错误映射和回调分发的基础骨架，新增 provider 必须优先复用，不得在业务 Service 中散落供应商原文错误。
- 魔方财务/魔方云和主机面板缓存 key 必须 provider-aware；历史旧 key 采用“双读旧键、写新键”迁移，不得把 `mofang_finance_api` 归一化为 `hosting_panel_api`。
- 对象存储等能力域真实 provider 尚未完成接入，新增改造必须按本节接口 + DTO + registry 模式推进。
...
- 后端文档：`文档/开发文档/后端/后端规范化工程化治理方案.md`。
...
1. 第一阶段：体检并列出风险，不直接重写。
2. 第二阶段：先修 Blocker 和 High，尤其是安全、资金、回调、越权、事务问题。
3. 第三阶段：抽象第三方接口和 DTO，引入 fake provider 测试。
4. 第四阶段：按支付、实名认证、魔方财务/魔方云顺序逐步剥离强耦合。
5. 第五阶段：把稳定规则沉淀到文档、skills 和测试模板。
