...
- 适用范围：`backend` 下所有返回 JSON 的 HTTP API，主要覆盖 `/api/admin/*`、`/api/client/*`、`/api/site/*` 以及直接重构新增的 `/api/v2/admin/*`、`/api/v2/client/*`、`/api/v2/site/*`
...
2. `AGENTS.md`
3. `文档/开发文档/后端/API直接重构方案.md`
4. 本文档
5. 旧接口习惯或历史蓝图

注意：当前开发文档、开发规范和其他约束性文档不具备完全真实性。API 治理必须以当前代码检查为主，发现本文档与当前代码冲突时，先记录冲突，再按检查结论更新本文档或相关实现。

接口审查和 v2 重构时，`api-docs/` 只作为旧接口快照与审查证据；v2 目标结构以 `API直接重构方案.md` 和本文为准。

### 1.1 直接重构边界

API 直接重构采用“旧接口冻结 + 新接口直接重构”的边界：

- 旧接口：现有 `/api/admin/*`、`/api/client/*`、`/api/site/*` 保持当前契约，不为了新规范做结构兼容、双字段兼容或参数兼容；只允许修复安全漏洞、明确 bug 和不改变契约的内部实现问题。
- 新接口：优先落在 `/api/v2/admin/*`、`/api/v2/client/*`、`/api/v2/site/*`，按本文和 `API直接重构方案.md` 一次性采用新规范。
- 前端迁移：按页面或业务域完整迁移到新接口，不在同一页面同时消费旧结构和新结构。
- 文档维护：旧接口文档可标记 legacy；新接口文档必须重新描述请求、响应、测试和字段边界。
...
- 旧接口保持现状，不为了新规范新增或删除兼容参数。
- 新接口禁止接受 `per_page`、`pageSize`、`limit/offset` 作为标准分页参数，除非该接口不是分页列表而是明确的业务限制参数，例如监控点数量上限。
- 新接口分页返回结构只能使用 `page_size`，不得额外返回 `per_page`。
- 前端迁移到新接口时必须同步改为 `page`、`page_size`。
...
- 路由清单变化后，重刷 `文档/开发文档/后端/后端API清单.md`
