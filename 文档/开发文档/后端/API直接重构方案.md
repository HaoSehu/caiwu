...
- 文档性质：重构执行方案
- 适用范围：`api-docs/` 覆盖的 admin、client、official 全量 HTTP API
- 基准来源：`api-docs/README.md` 及其 358 个接口文档
- 决策口径：旧接口保留冻结，新接口直接重构，不做旧结构兼容
- 更新时间：2026-07-05
...
本方案是 API 直接重构的任务真源；`api-docs/` 是旧接口快照和审查证据，不是 v2 新接口的目标结构。每个接口开工前必须回查旧接口文档、路由、Controller、FormRequest、Resource、Service。

## 1. 决策结论

本次 API 治理采用“旧版本冻结 + 新版本直接重构 + 前端逐页迁移”的方式。

旧接口不做结构兼容、不新增双字段、不在旧接口内加入 `include`、`fields`、`profile` 等兼容控制。旧接口只允许修复安全漏洞和明确 bug。所有结构性治理落到新 API。
...
如项目后续决定不显式使用 `/v2`，也必须保证新旧路由并存，旧路由冻结，新路由按本文规范重新设计。

## 2. 审查范围与关键数据

`api-docs/README.md` 当前记录：
...
本方案基于接口文档字段、返回示例和现有项目规则形成。执行前必须回查对应路由、Controller、FormRequest、Resource、Service，不允许凭文档猜测实现。
...
涉及 API 文档或清单重建时，按项目规则执行对应导出脚本，不手工编辑自动生成的 `文档/开发文档/后端/后端API清单.md`。
