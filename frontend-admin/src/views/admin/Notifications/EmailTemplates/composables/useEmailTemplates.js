...
export const ADMIN_TEMPLATE_CODES = new Set(['100010', '100011', '100013', '100014'])
...
export function useEmailTemplates() {
  ...
  function convertPlainTextToHtml(content) {
    ...
    if (!normalized) return '<p class="mail-empty">暂无邮件内容</p>'
    return normalized
      .split(/\n{2,}/)
...
<head>
  ...
  <style>
    body { margin: 0; padding: 0; background: #f3f4f6; font-family: "PingFang SC", "Microsoft YaHei", Arial, sans-serif; color: #1f2329; }
    .mail-shell { width: 100%; padding: 32px 12px; box-sizing: border-box; background: #f3f4f6; }
    .mail-card { width: 100%; max-width: 680px; margin: 0 auto; background: #ffffff; border: 1px solid #cfd6e4; overflow: hidden; }
    .mail-header { display: flex; align-items: flex-start; padding: 24px 28px 20px; border-top: 4px solid #1f4b99; border-bottom: 1px solid #d9e0ec; background: #f8fafc; }
    .mail-brand strong { display: block; font-size: 18px; line-height: 1.3; letter-spacing: 0.02em; color: #162033; }
    .mail-brand span { display: block; margin-top: 6px; font-size: 12px; color: #5b6575; }
    .mail-body { padding: 28px; }
    .mail-title { margin: 0; font-size: 28px; line-height: 1.4; color: #162033; }
    .mail-summary { margin: 12px 0 0; font-size: 14px; line-height: 1.8; color: #4b5565; }
    .mail-divider { height: 1px; margin: 24px 0; background: #d9e0ec; }
    .mail-content { font-size: 14px; line-height: 1.85; color: #1f2329; }
    .mail-content p { margin: 0 0 14px; }
    .mail-content p:last-child { margin-bottom: 0; }
    .mail-content strong { color: #162033; }
    .mail-content a { color: #1f4b99; text-decoration: underline; }
    .mail-content .mail-panel { margin: 18px 0; padding: 16px 18px; border: 1px solid #d9e0ec; background: #f8fafc; }
    .mail-content .mail-code { display: inline-block; margin: 8px 0 16px; padding: 14px 18px; border: 1px solid #1f4b99; background: #eef4ff; color: #1f4b99; font-size: 28px; line-height: 1; font-weight: 700; letter-spacing: 0.18em; }
    .mail-content .mail-kv { display: flex; align-items: flex-start; justify-content: space-between; gap: 16px; padding: 10px 0; border-bottom: 1px solid #d9e0ec; }
    .mail-content .mail-kv:first-child { padding-top: 0; }
    .mail-content .mail-kv:last-child { padding-bottom: 0; border-bottom: none; }
    .mail-content .mail-kv span { color: #4e5969; white-space: nowrap; }
    .mail-content .mail-kv strong { text-align: right; word-break: break-word; }
    .mail-content .mail-button { display: inline-block; margin-top: 12px; padding: 12px 18px; border: 1px solid #1f4b99; background: #1f4b99; color: #ffffff; font-weight: 600; text-decoration: none; }
    .mail-content .mail-muted, .mail-footer { color: #6b7280; font-size: 12px; line-height: 1.8; }
    .mail-footer { padding: 0 28px 28px; }
    .mail-empty { color: #86909c; }
    @media screen and (max-width: 640px) {
      .mail-shell { padding: 18px 10px; }
      .mail-header, .mail-body, .mail-footer { padding-left: 18px; padding-right: 18px; }
      .mail-title { font-size: 22px; }
      .mail-content .mail-kv { display: block; }
      .mail-content .mail-kv strong { display: block; margin-top: 6px; text-align: left; }
    }
  </style>
</head>
<body>
    ...
    try {
      ...
      data.forEach((item) => {
        if (String(item.key).startsWith('email_template_subject_')) {
          ...
          if (template) template.subject = item.value || template.defaultSubject
        }
        if (String(item.key).startsWith('email_template_content_')) {
          const code = String(item.key).replace('email_template_content_', '')
          const template = findTemplate(code)
          if (template) template.content = item.value || template.defaultContent
        }
      })
    } catch {
