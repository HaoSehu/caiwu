<template>
  <div class="admin-log-page">
    ...
    <el-card shadow="never" class="panel-card">
      <template #header>
        <div class="table-header">
          <div>
            <h3>系统日志明细</h3>
            <p>共 {{ pagination.total }} 条记录，支持查看原始日志内容。</p>
          </div>
        </div>
      </template>
      ...
      <div class="table-scroll-shell">
        <el-table v-loading="loading" :data="tableData" border row-key="id" class="admin-log-table system-log-table">
          ...
          <el-table-column label="日志内容" min-width="540" show-overflow-tooltip>
            <template #default="{ row }">
    ...
    <el-drawer v-model="detailVisible" title="系统日志详情" size="700px">
...
const {
  ...
  query: async ({ filters, pagination }) => {
    ...
    const [listRes, summaryRes] = await Promise.all([
      ...
      getSystemLogsSummary(compactParams({
        level: filters.level,
        keyword: filters.keyword,
        ...buildDateRangeParams(filters.dateRange),
      })),
    ...
    return {
      ...
      extra: {
        summary: summaryRes.data || { total: 0, errors: 0, warnings: 0, infos: 0 },
      },
    }
  },
  onSuccess: ({ rows, extra }) => {
    tableData.value = rows || []
    summary.value = extra?.summary || { total: 0, errors: 0, warnings: 0, infos: 0 }
  },
  onError: (error) => {
