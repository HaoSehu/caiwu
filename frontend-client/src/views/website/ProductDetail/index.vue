...
import { buildBreadcrumbJsonLd, buildProductJsonLd, useSeo } from '@/composables/useSeo'
...
async function ensureClientCanCreateOrder(orderPayload, idempotencyKey) {
  ...
  const missingRequirements = resolveMissingPurchaseRequirements(product.value, userInfo)
...
useSeo(() => {
  ...
  const canonicalPath = p.id ? `/products/${p.id}` : '/products'
  const displayName = selectedMachineSpec.value.displayName || resolveProductDisplayName(p)
  const title = p.meta_title || displayName
  const description = p.meta_description || `${displayName}：灵活配置、按需付费，支持快速开通与在线管理。`
  const keywords = p.meta_keywords
    || [displayName, p.type_label, detailGroup.value?.name, '创欧云'].filter(Boolean).join(',')
  const price = Array.isArray(pricingEntries.value) && pricingEntries.value.length
    ? pricingEntries.value[0]?.amount
    : undefined
  const origin = typeof window !== 'undefined' ? window.location.origin : ''
  const absolute = (path) => (origin ? new URL(path, origin).href : path)

  const breadcrumbItems = [{ name: '首页', url: absolute('/') }, { name: '产品中心', url: absolute('/products') }]
  const typeLabel = String(p.type_label || '').trim()
  const parentGroupName = String(detailGroup.value?.parent_name || '').trim()
  const groupName = String(detailGroup.value?.name || '').trim()

  if (typeLabel && typeLabel !== groupName && typeLabel !== parentGroupName) {
    breadcrumbItems.push({ name: typeLabel, url: absolute('/products') })
  }
  if (parentGroupName && parentGroupName !== groupName) {
    breadcrumbItems.push({ name: parentGroupName, url: absolute('/products') })
  }
  if (groupName) {
    breadcrumbItems.push({ name: groupName, url: absolute('/products') })
  }
  breadcrumbItems.push({ name: displayName || p.name, url: absolute(canonicalPath) })
