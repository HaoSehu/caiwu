  ...
  <div class="shop-wrap">
    <div v-if="showMobileTypePicker" class="mobile-type-stage" v-loading="pageLoading">
      <section class="mobile-stage-card mobile-stage-card--intro">
        <h1 class="mobile-stage-title">创欧云产品中心 · 选择一级菜单</h1>
      ...
      <div class="mobile-type-grid">
        ...
        >
          <div class="mobile-type-card-head">
            <span class="item-abbr">{{ type.abbr || type.label.slice(0, 2).toUpperCase() }}</span>
            <span class="mobile-type-count">{{ type.product_count || 0 }} 个商品</span>
          </div>
          <div class="mobile-type-name">{{ type.label }}</div>
          <p class="mobile-type-desc">{{ type.description || `进入 ${type.label} 子目录后可继续选择分类、商品和具体配置。` }}</p>
    ...
    <template v-else>
      <!-- 左侧产品目录侧边栏 -->
      <aside v-if="!isMobile" class="shop-sidebar" :class="{ collapsed: sidebarCollapsed }">
        <div class="sidebar-head">
          <span class="sidebar-head-text" v-show="!sidebarCollapsed">产品目录</span>
        ...
        >
          ...
          <template v-if="!sidebarCollapsed">
            <span class="item-body">
              ...
              <span class="item-sub">{{ type.product_count > 0 ? `${type.product_count} 个商品` : '暂无商品' }}</span>
            </span>
      ...
      <div class="shop-body">
        ...
        <div class="shop-main" v-loading="pageLoading">
          <div v-if="isMobile" class="mobile-picker-row mobile-picker-row--duo">
            <div class="mobile-picker-col">
              ...
              <button type="button" class="mobile-picker-trigger" @click="openMobileRegionDrawer">
                <span>{{ activeGroupName || '请选择地区' }}{{ activeChildName ? ` · ${activeChildName}` : '' }}</span>
            ...
            <div v-if="visibleProducts.length" class="mobile-picker-col">
              ...
              <button type="button" class="mobile-picker-trigger" @click="openMobileSpecPicker">
          ...
          <div v-if="selectedProduct && productIntroText && !isMobile" class="product-intro-card">
            <div class="config-block-title">商品介绍</div>
            <p>{{ productIntroText }}</p>
          </div>

          <!-- 商品配置区 -->
          <div class="config-area" v-if="selectedProduct" v-loading="configLoading">

          <!-- 操作系统 -->
          <div v-if="osGroups.length && isMobile" class="mobile-picker-row">
            <div class="config-block-title">操作系统</div>
            <button type="button" class="mobile-picker-trigger" @click="openMobileOsDrawer">
              <span>{{ currentOsGroup?.label || '请选择系统' }}{{ currentOsVersionLabel ? ` · ${currentOsVersionLabel}` : '' }}</span>
              <svg viewBox="0 0 12 12" fill="none" width="12" height="12" aria-hidden="true">
                <path d="M3 4.5l3 3 3-3" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
              </svg>
            </button>
          </div>
          <div class="config-block" v-if="osConfig && !isMobile && !machineConfigs.length">
            <template>
                ...
                <div v-if="osGroups.length" class="os-cards">
                  ...
                  >
                    <div class="os-card-head">
                      <img v-if="os.icon" :src="os.icon" :alt="os.label" class="os-logo-img" width="28" height="28" />
                      <span v-else class="os-logo">{{ os.label.slice(0, 2) }}</span>
                      <span class="os-label-name">{{ os.label }}</span>
                    </div>
                    <div class="os-card-ver">
                      <span v-if="configForm.os_group === os.id">{{ currentOsVersionLabel }}</span>
                      <span v-else class="os-placeholder">选择版本</span>
                    </div>
                    <span class="os-check-mark" v-if="configForm.os_group === os.id">✓</span>
                  </button>
                </div>
                <div v-else class="spec-fixed">当前商品未提供可切换的系统版本</div>
          ...
          <!-- 机型配置 -->
          <div v-if="(machineConfigs.length || hasMobileProductGroups) && isMobile">
            <div v-if="hasMobileProductGroups" class="mobile-picker-row mobile-picker-row--duo">
              <div class="mobile-picker-col">
                <div class="config-block-title">CPU</div>
                <button type="button" class="mobile-picker-trigger" @click="openMobileGroupedCpuDrawer">
              ...
              <div class="mobile-picker-col">
                ...
                <button type="button" class="mobile-picker-trigger" @click="openMobileGroupedMemoryDrawer">
            ...
            <div v-else-if="cpuConfig || memConfig" class="mobile-picker-row mobile-picker-row--duo">
              <div v-if="cpuConfig" class="mobile-picker-col">
                <div class="config-block-title">{{ cpuConfig.label }}</div>
                <button type="button" class="mobile-picker-trigger" @click="openMobileCpuDrawer">
                  <span>{{ cpuConfig.options.find(o => o.id === configForm[cpuConfig.key])?.label || '请选择' }}</span>
                  <svg viewBox="0 0 12 12" fill="none" width="12" height="12" aria-hidden="true">
                    <path d="M3 4.5l3 3 3-3" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                  </svg>
                </button>
              ...
              <div v-if="memConfig" class="mobile-picker-col">
                ...
                <button type="button" class="mobile-picker-trigger" @click="openMobileMemDrawer">
                  <span>{{ memConfig.options.find(o => o.id === configForm[memConfig.key])?.label || '请选择' }}</span>
                  <svg viewBox="0 0 12 12" fill="none" width="12" height="12" aria-hidden="true">
                    <path d="M3 4.5l3 3 3-3" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                  </svg>
                </button>
            ...
            <div v-if="otherMachineConfigs.length" class="config-block">
              <div class="spec-list">
                <template v-for="cfg in otherMachineConfigs" :key="cfg.key">
                  ...
                  >
                    <span class="mobile-config-select-label">{{ cfg.label }}</span>
                  ...
                  <div v-else class="spec-row">
                    ...
                    <div class="opt-row" v-else-if="cfg.options.length > 1">
                      <button v-for="opt in cfg.options" :key="opt.id" type="button" class="opt-btn" :class="{ active: configForm[cfg.key] === opt.id }" @click="configForm[cfg.key] = opt.id">{{ opt.label }}</button>
                    </div>
                    <span v-else class="spec-fixed">{{ cfg.options[0]?.label }}</span>
                  </div>
                </template>
              </div>
            </div>
          ...
          <div class="config-block" v-if="machineConfigs.length && !isMobile">
            ...
            <div v-if="desktopMachineSpecRows.length" class="desktop-machine-spec-panel">
              <div class="desktop-machine-table" role="table" aria-label="机型配置">
                <div class="desktop-machine-table-head" role="row">
                  <span class="machine-spec-cell machine-spec-cell--name">实例规格</span>
                  <span class="machine-spec-cell machine-spec-cell--sortable">
                    <span>vCPU</span>
                    <span class="machine-spec-sort-controls" aria-label="vCPU 排序">
                      <button
                        type="button"
                        class="machine-spec-sort-btn"
                        :class="{ active: machineSpecSort.key === 'cpu' && machineSpecSort.direction === 'asc' }"
                      ...
                      <button
                        type="button"
                        class="machine-spec-sort-btn"
                        :class="{ active: machineSpecSort.key === 'cpu' && machineSpecSort.direction === 'desc' }"
                  ...
                  <span class="machine-spec-cell machine-spec-cell--sortable">
                    <span>内存</span>
                    <span class="machine-spec-sort-controls" aria-label="内存排序">
                      <button
                        type="button"
                        class="machine-spec-sort-btn"
                        :class="{ active: machineSpecSort.key === 'memory' && machineSpecSort.direction === 'asc' }"
                        aria-label="内存从小到大排序"
                      ...
                      <button
                        type="button"
                        class="machine-spec-sort-btn"
                        :class="{ active: machineSpecSort.key === 'memory' && machineSpecSort.direction === 'desc' }"
                        aria-label="内存从大到小排序"
                  ...
                  <span class="machine-spec-cell machine-spec-cell--processor">处理器</span>
                  <span class="machine-spec-cell">主频/睿频</span>
                  <span class="machine-spec-cell machine-spec-cell--price machine-spec-cell--sortable">
                    <span>基础价格</span>
                    <span class="machine-spec-sort-controls" aria-label="基础价格排序">
                      <button
                        type="button"
                        class="machine-spec-sort-btn"
                        :class="{ active: machineSpecSort.key === 'price' && machineSpecSort.direction === 'asc' }"
                      ...
                      <button
                        type="button"
                        class="machine-spec-sort-btn"
                        :class="{ active: machineSpecSort.key === 'price' && machineSpecSort.direction === 'desc' }"
                ...
                >
                  ...
                  <span class="machine-spec-cell machine-spec-cell--processor">{{ row.processorLabel }}</span>
                  <span class="machine-spec-cell">{{ formatFrequencyPair(row.baseFrequency, row.turboFrequency) }}</span>
                  <span class="machine-spec-cell machine-spec-cell--price">{{ row.basePriceText }}</span>
                </button>
              </div>
              ...
              <div v-if="desktopMachineExtraConfigs.length" class="desktop-machine-extra-list">
                <template v-for="cfg in desktopMachineExtraConfigs" :key="cfg.key">
                  <MobileRangePicker
                    v-if="cfg.isNumber"
                    v-model="configForm[cfg.key + '_num']"
                    :label="cfg.label"
          ...
          <el-empty
            v-else-if="!pageLoading && !configLoading && !selectedProduct && visibleProducts.length"
            description="请选择需要使用优惠券的商品"
            style="padding: 60px 0"
          />

          <el-empty
            v-if="!pageLoading && !configLoading && !selectedProduct && !visibleProducts.length"
        ...
        <aside class="shop-cost" v-if="selectedProduct">
          ...
          <div class="cost-detail" :class="{ 'cost-detail--loading': quoteLoading }">
            <div class="cost-item">
              <span>产品</span><span>{{ selectedProductSummaryName || selectedProductDisplayName }}</span>
            </div>
          ...
          <div class="cost-breakdown" :class="{ 'cost-breakdown--loading': quoteLoading }">
            ...
            <div class="cost-item cost-item--discount" v-if="appliedCoupon">
              <span>优惠券 {{ appliedCoupon.code }}</span><span>-¥{{ appliedCoupon.discount_amount }}</span>
            </div>
          </div>
          ...
          <div class="coupon-panel">
            ...
            <div v-else-if="!availableCoupons.length" class="coupon-panel-tip coupon-panel-tip--muted">
              {{ selectedProduct ? '当前暂无可用优惠券，登录后如有优惠券会自动展示在这里。' : '请选择商品后查看可用优惠券。' }}
            </div>
          </div>
          ...
          <button
            ...
            @click="handleSubmit"
          >
            <span>{{ soldOut ? '已售罄' : (submitting ? '提交中...' : '立即购买') }}</span>
          </button>
        </aside>
      </div>
        ...
        <div ref="allocationFooterInnerEl" class="allocation-footer-inner">
          <div class="allocation-footer-main">
            <div class="allocation-footer-summary">
              ...
              <span class="allocation-footer-discount-text">
                {{ appliedCoupon ? `已优惠 ¥${appliedCoupon.discount_amount}` : '无折扣' }}
              </span>
            </div>
            <div class="allocation-footer-meta">
              <span v-if="stockLabel">{{ stockLabel }}</span>
              <span v-if="selectedCycleLabel">{{ selectedCycleLabel }}</span>
              <button type="button" class="allocation-footer-coupon-btn" @click="mobileCouponDrawer = true">
                <el-icon class="allocation-footer-coupon-icon"><Ticket /></el-icon>
                {{ appliedCoupon ? `使用优惠券 -¥${appliedCoupon.discount_amount}` : '使用优惠券' }}
              </button>
            </div>
          </div>
          <div class="allocation-footer-actions">
            <button
              ...
              @click="handleSubmit"
            >
              <span>{{ soldOut ? '已售罄' : (submitting ? '提交中...' : '加入购物车') }}</span>
            </button>
          </div>
          <div class="allocation-footer-links">
            <button v-if="productIntroText" type="button" class="allocation-footer-detail-btn" @click="mobileIntroDrawer = true">
              <el-icon class="allocation-footer-detail-icon"><Document /></el-icon>
              商品介绍
            </button>
            <button type="button" class="allocation-footer-detail-btn" @click="mobileCostDrawer = true">
              <el-icon class="allocation-footer-detail-icon"><Document /></el-icon>
              费用明细
            </button>
          </div>
      ...
      <MobileSheet v-model="mobileIntroDrawer" size="40%" title="商品介绍" confirm-text="关闭" @confirm="mobileIntroDrawer = false">
        <div class="mobile-intro-sheet-body">
          <p>{{ productIntroText }}</p>
        </div>
      </MobileSheet>

      <MobileSheet v-model="mobileCostDrawer" size="78%" title="费用明细" confirm-text="关闭" @confirm="mobileCostDrawer = false">
        <div class="mobile-cost-sheet-body">
          ...
          <div class="mobile-cost-table-card">
            ...
            <div class="mobile-cost-table">
              ...
              <div class="mobile-cost-table-row" v-for="row in mobileCostRows" :key="row.key">
      ...
      <MobileSheet v-model="mobileCouponDrawer" size="42%" title="优惠券" confirm-text="关闭" @confirm="mobileCouponDrawer = false">
        ...
        <div class="mobile-drawer-sheet">
          <div class="mobile-drawer-head">
            <div>
              ...
              <p class="mobile-drawer-desc">{{ activeTypeLabel || '当前一级菜单' }}</p>
            </div>
            <button type="button" class="mobile-drawer-switch" @click="returnToMobileTypePicker">切换一级菜单</button>
          </div>
      ...
      <MobileSheet v-model="mobileProductDrawer" size="40%" title="请选择产品规格" cancel-text="取消" confirm-text="确定" @confirm="confirmMobileProductSelection">
        <div class="mobile-product-sheet-list">
          ...
          <template v-else>
            ...
            >
              <span class="mobile-product-sheet-name-wrap">
                <span class="mobile-product-sheet-name">{{ resolveProductDisplayName(p) }}</span>
                <el-popover
                ...
                >
                  ...
                  <div class="spec-note-content">{{ normalizeInstanceSpecNote(p.instance_spec_note) }}</div>
                </el-popover>
              </span>
              <span v-if="formatProductListPrice(p)" class="mobile-product-sheet-price">{{ formatProductListPrice(p) }}</span>
            </button>
          </template>
        </div>
      </MobileSheet>
...
import { CaretBottom, CaretTop, Document, Expand, Fold, Ticket } from '@element-plus/icons-vue'
...
function setupAllocationFooterObserver() {
  ...
  if (!isMobile.value || !selectedProduct.value || !allocationFooterEl.value || !('IntersectionObserver' in window)) {
    return
  }

  allocationFooterObserver = new IntersectionObserver(([entry]) => {
  ...
  if ('ResizeObserver' in window && allocationFooterInnerEl.value) {
    allocationFooterResizeObserver = new ResizeObserver(([entry]) => {
      ...
      if (height > 0) {
        allocationFooterEl.value?.style.setProperty('--allocation-footer-height', `${height}px`)
      }
    })
...
function formatProductListPrice(product) {
  ...
  const monthlyEntry = pricingEntries.find((item) => item?.cycle === 'monthly' && Number(item?.amount || 0) > 0)
  const fallbackEntry = monthlyEntry || pricingEntries.find((item) => Number(item?.amount || 0) > 0)
...
function resolveMobileProductSpec(product) {
  ...
  const explicitSpecText = String(sourceProduct.instance_spec_text || sourceProduct.instance_spec_alias || '').trim()
...
const mobileCostRows = computed(() => {
  const rows = []
  const productBaseAmount = Number(baseAmount.value || 0) + Number(setupFee.value || 0)

  if (selectedProduct.value) {
    rows.push({
      key: 'product',
      label: '配置名称',
      detail: selectedProductSummaryName.value || resolveProductDisplayName(selectedProduct.value),
      amount: formatMoneyText(productBaseAmount),
    })
  }

  summaryItems.value.forEach((item) => {
    const matchedQuote = quoteItems.value.find((quoteItem) => quoteItem.label === item.label)
    rows.push({
      key: item.key,
      label: item.label,
      detail: item.value,
...
const selectedProductGroup = computed(() => selectedProduct.value?.group || null)
const productIntroText = computed(() => String(
  selectedProductGroup.value?.slogan
  || ''
).trim())
...
const otherMachineConfigs = computed(() => machineConfigs.value.filter((cfg) => !isCpuConfig(cfg) && !isMemConfig(cfg)))
...
const selectedProductSummaryName = computed(() => {
  ...
  const sourceProduct = mergeProductPresentationSource(listProduct || selectedProduct.value, selectedProduct.value)
  const displayName = resolveProductDisplayName(sourceProduct)
  const specSource = String(sourceProduct.instance_spec_text || sourceProduct.instance_spec_alias || displayName || '').trim()
  const cpuMemoryDisplay = String(sourceProduct.cpu_memory_display || '').trim()
  const configSpec = resolveMachineSpecSelection(sourceProduct.config_options, {
...
const mobileProductSpecFamilies = computed(() => {
  ...
  return Array.from(families.values()).sort((left, right) => left.originalIndex - right.originalIndex)
})
...
const hasMobileProductGroups = computed(() => (
  ...
  && mobileProductSpecRows.value.every((row) => row.baseName && row.cpuSlug && row.memorySlug)
))
const selectedMobileSpecFamily = computed(() => {
  ...
  return mobileProductSpecFamilies.value.find((family) => family.baseName === selectedGroup?.baseName)
...
const selectedMobileProductGroup = computed(() => {
  ...
  return mobileProductGroups.value.find((group) => (
...
const selectedMobileMemoryLabel = computed(() => {
  const selectedId = Number(selectedProductId.value || 0)
  const matched = selectedMobileProductGroup.value?.products.find((row) => row.productId === selectedId)
  return matched?.memoryText || ''
})
const mobileGroupedCpuOptions = computed(() => (
  mobileProductGroups.value.filter((group) => (
    !selectedMobileSpecFamily.value || group.baseName === selectedMobileSpecFamily.value.baseName
  )).map((group) => ({
    id: group.key,
...
const mobileGroupedMemoryOptions = computed(() => (
  selectedMobileProductGroup.value?.products.map((row) => ({
    ...
    label: row.memoryText || row.memorySlug || resolveProductDisplayName(row.product),
  })) || []
))
...
async function openMobileProductGroupDrawer() {
  ...
  if (hasMobileProductGroups.value) {
    mobilePendingProductGroupKey.value = selectedMobileProductGroup.value?.key || mobileProductGroups.value[0]?.key || ''
  } else {
...
function findPreferredMobileProductId(group) {
  ...
  const selectedId = Number(selectedProductId.value || 0)
  const currentMemory = mobileProductSpecRows.value.find((row) => row.productId === selectedId)?.memorySlug || ''
...
function findPreferredMobileGroup(groups) {
  ...
  const selectedId = Number(selectedProductId.value || 0)
  const currentRow = mobileProductSpecRows.value.find((row) => row.productId === selectedId)
  const currentCpuSlug = currentRow?.cpuSlug || ''
  const matchedCpu = currentCpuSlug
    ? groups.find((group) => group.products.some((row) => row.cpuSlug === currentCpuSlug))
    : null
...
function confirmMobileGroupedCpuSelection(groupKey) {
  ...
  const targetGroup = mobileProductGroups.value.find((group) => group.key === groupKey)
...
function confirmMobileSpecFamilySelection(familyKey) {
  ...
  const targetFamily = mobileProductSpecFamilies.value.find((family) => family.key === familyKey)
  if (!targetFamily) {
    return
  }

  const targetGroups = mobileProductGroups.value.filter((group) => group.baseName === targetFamily.baseName)
...
function confirmMobileProductSelection() {
  if (hasMobileProductGroups.value) {
    const targetGroup = mobileProductGroups.value.find((group) => group.key === mobilePendingProductGroupKey.value)
...
useSeo(() => {
  ...
  const typeLabel = activeTypeLabel.value || '云服务器'
  const groupName = activeGroupName.value || ''
  const productName = resolveProductDisplayName(product)
  const pieces = [productName, groupName, typeLabel].filter(Boolean)
  const title = pieces.length ? pieces.slice(0, 2).join(' · ') : '产品与服务'
  const description = product?.meta_description
    || `创欧云提供多区域 ${typeLabel}，支持灵活配置与快速开通，覆盖香港、美国与国内多地节点，适合云计算与 IDC 业务。`
  const keywords = product?.meta_keywords
    || [productName, typeLabel, groupName, '云服务器', 'IDC', '创欧云'].filter(Boolean).join(',')
  const canonical = product?.id ? `/products/${product.id}` : '/products'
