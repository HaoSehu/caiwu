...
export function useSecurityDialogs(form, postCodeWithFallback, runWithCaptcha, userStore) {
  ...
  async function handleChangePhone() {
    ...
    try {
      ...
      if (!error?.__handled) ElMessage.error(error.message || '修改失败')
    } finally {
  ...
  async function handleSendEmailCode() {
    ...
    try {
      ...
      if (!error?.__handled) ElMessage.error(error.message || '邮箱验证码发送失败')
    } finally {
  ...
  async function handleVerification() {
    ...
    try {
      ...
      if (!error?.__handled) ElMessage.error(error.message || '提交失败')
    } finally {
  ...
  async function handleRestartVerification() {
    ...
    try {
      ...
      if (!error?.__handled) ElMessage.error(error.message || '重新认证失败')
  ...
  async function refreshVerificationLink() {
    ...
    try {
      ...
      if (!error?.__handled) ElMessage.error(error.message || '获取认证链接失败')
    } finally {
      verificationLoading.value = false
    }
  }
