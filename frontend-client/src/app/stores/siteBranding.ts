...
const DEFAULT_SITE_DESCRIPTION = '创欧云 — 稳定、安全、高性价比的云服务器与 IDC 服务平台，覆盖香港、美国与国内多地节点，提供云服务器、独立服务器、云电脑与工单支持。'
const DEFAULT_SITE_KEYWORDS = '云服务器,独立服务器,高防服务器,云电脑,香港服务器,美国服务器,BGP 多线,IDC 服务,创欧云'
const DEFAULT_ROBOTS_DIRECTIVE = 'index,follow'
const DEFAULT_CANONICAL_BASE = String(import.meta.env.VITE_SITE_CANONICAL_BASE || '').replace(/\/$/, '')
...
export const useSiteBrandingStore = defineStore('site-branding', () => {
  ...
  const siteDescription = ref(DEFAULT_SITE_DESCRIPTION)
  const siteKeywords = ref(DEFAULT_SITE_KEYWORDS)
  ...
  const robotsDirective = ref(DEFAULT_ROBOTS_DIRECTIVE)
  const canonicalBase = ref(DEFAULT_CANONICAL_BASE)
  const termsUrl = ref('')
  const privacyUrl = ref('')
  const verifyGoogle = ref('')
  const verifyBaidu = ref('')
  const verifyBing = ref('')
  const verify360 = ref('')
  const verifySogou = ref('')
  ...
  function hydrateSiteConfig(data = {}) {
    ...
    siteDescription.value = pick(data, ['site_description'], siteDescription.value || DEFAULT_SITE_DESCRIPTION)
    siteKeywords.value = pick(data, ['site_keywords'], siteKeywords.value || DEFAULT_SITE_KEYWORDS)
    ...
    robotsDirective.value = pick(data, ['robots_directive'], robotsDirective.value || DEFAULT_ROBOTS_DIRECTIVE)
    canonicalBase.value = String(pick(data, ['canonical_base'], canonicalBase.value || '')).replace(/\/$/, '')
    verifyGoogle.value = pick(data, ['verify_google'], verifyGoogle.value || '')
    verifyBaidu.value = pick(data, ['verify_baidu'], verifyBaidu.value || '')
    verifyBing.value = pick(data, ['verify_bing'], verifyBing.value || '')
    verify360.value = pick(data, ['verify_360'], verify360.value || '')
    verifySogou.value = pick(data, ['verify_sogou'], verifySogou.value || '')
