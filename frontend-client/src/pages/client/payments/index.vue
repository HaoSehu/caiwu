    ...
    <div v-else class="mobile-record-list" v-loading="loading">
      <div v-for="row in list" :key="row.id" class="mobile-record-card" @click="goToInvoice(row)">
        <div class="card-row card-row--top">
          <span class="record-no">{{ row.payment_no || `#${row.id}` }}</span>
          <el-tag :type="resolvePaymentTagType(row.status)" size="small" effect="light">
            {{ row.status_label || '--' }}
          </el-tag>
        </div>
        <strong>{{ row.gateway_label || row.gateway || '--' }}</strong>
        <span>{{ row.trade_no || row.invoice_no || '等待渠道回调' }}</span>
        <div class="card-row">
          <span>充值：¥{{ row.amount || '0.00' }}</span>
