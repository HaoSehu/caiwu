...
export function resolveServiceStatusLabel(status: unknown) {
  ...
  const label = String(SERVICE_STATUS_MAP[serviceStatus]?.label || '').trim()
  return label !== '' ? label : '-'
}
...
export function resolveServiceName(item: AnyRecord) {
  return item?.custom_service_name || item?.name || item?.product_spec_display || item?.product_display_name || item?.product?.display_name || `服务 #${item?.id || 0}`
}
...
export function useServiceCenter() {
  ...
  const total = ref(0)
  const overview = shallowRef(createEmptyOverview())
  ...
  const statusOptions = [
    DEFAULT_SERVICE_STATUS_OPTION,
    ...toSelectOptions(SERVICE_STATUS_MAP, false),
  ]
  ...
  async function submitRenew() {
    ...
    try {
      ...
      ElMessage.error(error?.message || '续费账单创建失败')
    } finally {
  ...
  async function submitRemark() {
    ...
    try {
      ...
      if (index >= 0) {
        list.value.splice(index, 1, { ...list.value[index], ...updatedItem })
      }
