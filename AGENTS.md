...
- **每完成一个任务（或子任务）后，立即执行 git commit 并 push 到 GitHub**。commit message 采用 Google 项目风格（如 `Fix:修复无效格式`、`Feat:新增导出功能`、`Refactor:重构用户模块`），格式为 `Type:中文描述`，多个子任务各自独立提交，不要攒到最后一次性提交。
