...
- `shared`：跨端共享状态映射、展示组件与通用配置。前端通过 `@shared` 别名引用。
...
- 前端：Vue 3、Composition API、Vite 6、Element Plus、Pinia、axios、Sass。
...
- 统一 Element Plus，不引入第二套 UI 库。
- 接口请求走各端 `src/utils/request.js`，不要直接新建 axios 实例。
- 认证存取走 `src/utils/auth.js`，不要直接操作 localStorage 键。
...
- 新页面复用现有骨架类名：`admin-page`、`admin-page-head`、`admin-page-heading`、`admin-metric-strip`。
...
- **查表结构**：`文档/数据库/数据库结构说明-idc-2026-04-17.md`；疑难以实库 `information_schema` 与 `文档/架构/架构现状说明.md` 为准。
...
- `frontend-admin` 和 `frontend-client` 的 build 都会自动生成 `.gz` / `.br` 预压缩文件。
- `frontend-client` 的 `npm run build` 还会自动注入站点验证 meta。
- `frontend-client` 额外脚本：`build:with-sitemap`（构建+生成 sitemap）、`build:prerender`（构建+预渲染）、`verify:refactor`（源码健康检查+构建）、`check:source-health`（单独源码检查）。
